...
We write unified contexts as either $\Delta$ or $\Xi$, preferring the
latter when there is a chance of confusing them with linear contexts
$\Delta$. 

\bigskip
\begin{definition}[Unified context]
A {\em unified context} (written $\Xi$ or $\Delta$) 
over the domain $T$ is a finite map from a countably infinite
set of {\em variables} ($x$, $y$, $z$, etc.) to judgments
$\islvl{T}$, where $\mlvl$ is either $\mpers$, $\meph$, or $\mtrue$. 
If $\Xi$ maps $x$ to $\islvl{T}$, we say $\Xi$ {\em contains}
$x{:}\islvl{T}$. 
\end{definition}
\bigskip

\noindent
The metavariable $T$ represents an arbitrary domain; when
...
The {\it order} in ordered logic will comes from an injective function
$p(x)$ from variables onto some dense set (real numbers $\mathbb{R}$
will do just fine).  The function need not be 1-to-1 (if the
cardinality of the dense set is greater than the cardinality, this
would be impossible) but it's necessary that function respect density
in the sense that if $p(x) < p(y)$, we can find a variable $z$ where
$p(x) < p(z) < p(y)$. 

This formal account is compatible with the more traditional
representation of ordered
contexts as sequences
\[
\Xi ::= \cdot 
  \mid \Xi, x{:}\ispers{T}
  \mid \Xi, x{:}\iseph{T}
  \mid \Xi, x{:}\istrue{T}
\]
if we just stipulate that in a context of the form
$\Xi_L, x_1{:}\istrue{T_1}, \Xi_M, x_2{:}\istrue{T_2}, \Xi_R$, it must
be the case that $p(x_1) < p(x_2)$. 
By sorting out three subsequences of persistent, linear, and ordered
contexts, we can recover the usual presentations of contexts for
\ollll. We will use this to aid informally motivate our notation,
writing $\Xi = \Gamma; \Delta; \Omega$. 

\subsection{Fundamental operations on contexts}

The first fundamental constructor of contexts is $x{:}\islvl{T}$,
which is the map that contains $x{:}\islvl{T}$ and nothing else.
(Recall that $T$ is just $A$ in the unfocused setting and either $A^-$
or $\langle A^+ \rangle$ in the polarized, focused setting.)  The
...
Sole membership alone is related to the initial sequents and the 
matching construct $\Gamma; \cdot;/A/$ for contexts 
in ordered linear logic.
Specifically, we could have instead written the {\it init} rule in 
Figure~\ref{fig:ordered-prop} as
\[
\infer[{\it init}]
{\Xi \Rightarrow /{p}/}
{x{:}p \subseteq \Xi}
...
The second basic matching construct on a construct requires us to be
able to talk about context {\it frames}, which we write $\Theta$. 
Informally, if 

The second basic matching construct on a context is splitting
the context into an inside and an outside.

\bigskip
\begin{definition}
We say that $\Xi$ is equal to $\frameoff{\Theta}{\Xi'}$, where
$\Xi'$ is the {\em framed off} context and $\Theta$ is the {\em frame},
if 
\end{definition}
\bigskip
...
let $\Xi = \Gamma; \Delta'; \Omega$. In other words, saying that
$\Xi = \frameoff{\Theta}{\Xi'}$ means that $\Xi'$ is a piece of the inside
of $\Xi$ and $\Theta$ is the remaining outside; $\Xi'$ has been 
{\it framed out
