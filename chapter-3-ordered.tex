...
A sequent in the focusing calculus has the form $\foc{\Psi}{\Delta}{U}$, 
where $\Psi$ is the first-order context, $\Delta$ is a substructural
context as just described, and $U$ is a succeedent. The domain $T$ 
of the substructural context includes negative atomic propositions 
$A^-$, positive suspended propositions $\susp{A^+}$, focused negative
proposition $[A^-]$, and inverting positive propositions $A^+$. 

The form of the succedent $U$ is $\islvl{T}$, where $\mlvl$ is either
$\mtrue$ or $\mlax$;
In this way, $U$ is just a like a special substructural
context with exactly one element. The domain of $T$ for succedents is 
complementarly to the domain of $T$ for contexts.

A context is {\it stable} when it only includes negative atomic propositions
$A^-$ and positive suspended propositions $\susp{A^+}$, and we will restrict
our consideration to sequents 

Because we always know 


To describe a focused sequent caluclus for \ollll, we will instantiate 
the substructural contexts that we described so that $T$ can be one 
of four things:
\begin{itemize}
\item $A^-$ -- a negative atomic proposition (stable on the left)
\item $\langle A^+ \rangle$ -- a positive atomic proposition (always stable)
\item $[A^-]$ -- a left-focused negative proposition 
\item $A^+$ -- a positive inverting proposition 
\end{itemize}

is the negative propositions $A^-$ and suspended positive prositions
$\susp{A^+}$

The domain of our substructural contexts (written as 

\input{figs/fig-foc-mall}
\input{figs/fig-foc-add}

We present the focused sequent calculus for \ollll~in 
tandem with a proof term assignment, following the structural 
focalization methodlogy \cite{simmons11structural}. This will allow
for a greatly compressed discussion of cut admissibility and identity
expansion, and we will reuse much of the notation in our formulation
of a logical framework in the next chapter.

The goal of the proof term assignment here is to act as a true stand-in
for 
\begin{align*}
V & ::= z                     %% z
   \mid \tbangr{N}            %% !N
