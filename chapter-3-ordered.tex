...
\item[--] $(\subst{\tlaxr{N}}{\tlaxl{M}})^{{\ocircle}A^+}
  ...
  by part 3 of cut admissibility on $N$ and $M$.
...
\item[--] $\lsubsta{\texistsl{a}{N}}{M}{A^+} = \texistsl{a}{(\lsubsta{N}{M}{A^+})}$\smallskip\\
  ...
  and the required result then follows from rule $\exists_L$.
...
\paragraph{Substitution into values}
...
\item[--] $\rsubsta{M}{x}{(\tbangr{N})}{A^-}
  ...
  We must show
\item[--] $\rsubsta{M}{x}{\toner}{A^-} = \toner$
\item[--] $\rsubsta{M}{x}{(\tfuser{V_1}{V_2})}{A^-} = $\\
    $\begin{array}{ll}
