...
\item[--] $\lsubsta{\tbangl{x}{N}}{M}{A^+} 
  ...
  $\frameoff{\Theta}{\tackon{\Theta_B}{x{:}\ispers{B^-}}}$.\robnote{This,
    according to the definitions, is not technically true: $x$ is not
    a binding in $\Theta$ (weakening). I think the matching
    constructs need to be changed so that they {\it always} allow for
    weakening.}  By the induction hypothesis on $N$ and $M$ we have
  $\foc{\Psi}{\tackon{(\Theta {\circ}
      \Theta_B)}{x{:}\ispers{B^-}}}{U'}$, and the required
  result then follows from rule ${!}_L$. 
\item[--] $\lsubsta{\tupr{N}}{M}{A^+} = $
\item[--] $\lsubsta{\tlaxr{N}}{M}{A^+} = $
\item[--] $\lsubsta{\tfusel{N}}{M}{A^+} = $
\item[--] $\lsubsta{\tlaml{N}}{M}{A^+} = $
\item[--] $\lsubsta{\tlamr{N}}{M}{A^+} = $
\item[--] $\lsubsta{\tabort}{M}{A^+} = \tabort$
\item[--] $\lsubsta{\toplusl{N_1}{N_2}}{M}{A^+} = $
\item[--] $\lsubsta{\ttopr}{M}{A^+} = \ttopr$
\item[--] $\lsubsta{\twithr{N_1}{N_2}}{M}{A^+} = $
\item[--] $\lsubsta{\texistsl{a}{N}}{M}{A^+} = $
\item[--] $\lsubsta{\tforallr{a}{N}}{M}{A^+} = $
