...
\begin{itemize}
\item[--] $(\subst{z}{\tetap{z'}{N}})^{p^+_{\mlvl}} = [z/z']N$ --- We
  have a derivation $\foc{\Psi}{\Delta}{[p^+_\mlvl]}$ using the rule
  ${\it id}^+$; we know that $\Delta$ matches $x{:}\susp{p^+_\mlvl}$.

  Because $\Delta$ is suspension-normal, there exists a $\Delta'$ such
  that $\Delta$ matches $\restrictto{\Delta'}{\mlvl}$ where $\Delta'$
  matches $x{:}\susp{p^+_\mlvl}$ that we have a derivation of
  $\foc{\Psi}{\Delta'}{[p^+_\mlvl]}$ using the rule ${\it id}^+$ as
  well.\footnote{Strictly speaking, we have that $\Delta$ matches
  ...
  and $\Xi$ matches $\frameoff{\Theta}{\Delta}$; this means 
  $\Xi$ matches $\frameoff{\Theta}{\restrictto{\Delta}{\mlvl}}$
  as well, so the result $\foc{\Psi}{\Xi}{U}$ follows
  by focal substitution.

\item[--] $(\subst{\tdownr{M}}{\tdownl{x}{N}})^{{\downarrow}A^-} 
           = \rsubsta{M}{x}{N}{A^-}$ ---
  $M$ is a derivation of $\foc{\Psi}{\Delta}{A^-}$,
  $N$ is a derivation of $\foc{\Psi}{\tackon{\Theta}{x{:}\istrue{A^-}}}{U}$,
  and $\Xi$ matches $\frameoff{\Theta}{\Delta}$. 

  XXX

\futurework{I'm relying on a property I haven't stated yet: that 
if $\Xi$ matches $\frameoff{\Theta}{[A^-]}$ or
 $\frameoff{\Theta}{A^+}$, then $\tackon{\Theta}{[B^-]}$
 or $\tackon{\Theta}{B^+}$ will match $\frameoff{\Theta}{[B^-]}$
 and $\frameoff{\Theta}{B^+}$, respectively. Is it even stronger
 than that? We need to preserve the frame structure over two steps. }
...
\item[--] $(\subst{\texistsr{t}{V}}{\texistsl{a}{N}})^{\exists a{:}\tau.A^+}
           = (\subst{V}{[t/a]N})^{[t/a]A^+}$ --- XXX

\item[--] $(\subst{\tunifr}{\phi})^{t \doteq t} = \phi({\sf id})$ --- XXX 
\end{itemize}
...
\item[--] $(\subst{}{})^{{\uparrow}A^+}$

\item[--] $(\subst{}{})^{{\ocircle}A^+}$

\item[--] $(\subst{}{})^{A^+ \righti B^-}$

\item[--] $(\subst{}{})^{A^+ \lefti B^-}$

\item[--] $(\subst{}{})^{\top}$

\item[--] $(\subst{}{})^{A^- \with B^-}$

\item[--] $(\subst{}{})^{\forall x.A^-}$
\end{itemize}
