...
\}_{\Psi'}$.\footnote{The notaiton $\{ e \| \mu \}_\Psi$ is intended
  to invoke Harper's notation $\nu \Sigma \{ e \| \mu \}$, which is
  used to decribe mutable references and lazy evaluation in Chapters
...
and $v$ remain syntactic objects adequatly representable in LF, as
...
and commutiative operator (just like conjunction in linear logic).  A
...
represented in \sls. Nevertheless, the example above suggests that a
carefully-defined formalism for statefully-modular natural semantics
specifications could be similarly compiled into (or, indeed, {\it
  defined in terms of}) their operationalization as concurrent
\sls~specifications.
...
modular extension of small-step structural sperational semantics
specifications rather than big-step natural semantics, though Mosses
does discuss the latter. The operationization transformation applies
...
logical correspondance between SOS specficiations and ordered abstrct
...
programming language specification, {\it structural operational
  semantics} or SOS specifications. Small-step structural operational
semantics specifications define single-step evaluation inductively
over the structure of expressions. This is the SOS semantics for our 
...
Section~\ref{sec:trans-tail} when we operationalized the ${\sf
  ev}$ predicate only to motivate tail recursion. On the right-hand
side of Figure~\ref{fig:cbv-sos}, we show the specification that
results if we instead operationalize and defunctionalize 
