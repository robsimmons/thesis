...
\begin{itemize}
\item $\opbasic{{!}{\sf a}\,\lf{t^{\it in}_n}\,\lf{t_{n+1}}}{\lf{t_{n+1}}}{\sigma} 
...
Because 
${\sf r}{:}\forall\lf{\overline{x_0}}.\,{\sf eval\_a}\,\lf{t_0} \lefti \opbasic{A_1, \ldots, A_n}{t_{n+1}}{{\sf id}} \in \transop{\Sigma}$, by focusing
on that constant
it suffices to show that, for all $1 \leq i \leq n$, 
there is
\smallskip
\begin{itemize}
...
\leadsto^*_{\transop{\Sigma}}
 (\Psi; \tackon{\opsubst{\Theta}}{y{:}{\sf retn\_a}\,({\sigma_n}t_{n+1})})$.\footnote{The derived pattern+trace form is discussed at the end of Section~\ref{sec:framework-concurrent}.}
\end{itemize}
\smallskip
We will prove this
...
\item $\opbasic{{!}{\sf ac}\,\lf{t^{\it in}_n}\,\lf{t_{n+1}}}{t_{n+1}}{\sigma_{n-1}} 
  ...
  $P = x_n :: (\Psi; 
  \tackon{\opsubst{\Theta}}
  {{\sf eval\_a}\,(\sigma_{n}{t^{\it in}_n})}) \Longrightarrow_{\transop{\Sigma}}
   (\Psi; 
  \tackon{\opsubst{\Theta}}
  {x_{n}{:}{\susp{{\sf eval\_a}\,(\sigma_{n}{t^{\it in}_n})}}})$.
We need to construct a trace $(\Psi; 
  \tackon{\opsubst{\Theta}}
  {x_{n}{:}{\susp{{\sf eval\_a}\,(\sigma_{n}{t^{\it in}_n})}}})
  \leadsto^*_{\transop{\Sigma}} 
...
\item $\opbasic{{!}p^-_{ci},\ldots,{!}p^-_{cj},A^+_{j+1},\ldots,A^+_n}{t_{n+1}}{\sigma_{i-1}}$
  \\
  $~ \qquad = {\ocircle}({\sf eval\_bi}\,\lf{({\sigma_{i-1}}t^{\it in}_i)}
                    \fuse  \ldots \fuse
                 {\sf eval\_bj}\,\lf{({\sigma_{i-1}}t^{\it in}_j)} \fuse ~$
  \\
  $~ \qquad \qquad \quad (\forall\lf{\overline{x_i}}\ldots\forall\lf{\overline{x_j}}.\, 
...
\end{align*}
  ...
  $\slst{\transop{\Sigma}}{\Psi}
     ...
     {\Sp'}{\susp{{\ocircle}C^+}}$  \` (by inner ind. hyp.)
  \\
  $P' :: (\Psi, \tackon{\opsubst{\Theta}}{C^+}) \Longrightarrow_{\transop{\Sigma}}
        (\Psi, \tackon{\opsubst{\Theta}}{\Delta})$ 
   \` (by inner ind. hyp.)
  \\
  $T' :: (\Psi, \tackon{\opsubst{\Theta}}{\Delta}) \leadsto^*_{\transop{\Sigma}}
        (\Psi, \tackon{\opsubst{\Theta}}{y{:}\susp{{\sf retn\_a}\,s}})$ 
   \` (by inner ind. hyp.)
  \end{tabbing}
  We can conclude letting $T = 
...
\item $\opbasic{{!}G,A^+_{i+1},\ldots,A^+_n}{t_{n+1}}{\sigma_{i-1}} = \forall
  ...
  \` (by inner ind. hyp.)
  \\
  $P :: (\Psi; \tackon{\opsubst{\Theta}}{C^+}) \Longrightarrow^*_{\transop{\Sigma}}
   (\Psi; \tackon{\opsubst{\Theta}}{\Delta})$
  \` (by inner ind. hyp.)
  \\
  $T :: (\Psi; \tackon{\opsubst{\Theta}}{\Delta}) \leadsto^*_{\transop{\Sigma}}
...
S & ::= \forall \lf{x}{:}\tau.\,S 
   ...
   \mid {!}A^- \lefti S
   \mid \{ {\sf eval\_b1}\,\lf{t_1} \fuse \ldots \fuse {\sf eval\_bn}\,\lf{t_n} 
           \fuse {\downarrow}R \}
   \mid \{ {\sf eval\_b}\,\lf{t} \} 
\end{align*}
...
and 
$(\Psi; \tackon{\opsubst{\Theta}}{y{:}\susp{{\sf retn\_a}\,(\lf{\sigma t_{n+1}})}})
  \leadsto^*_{\transop{\Sigma}}
 (\Psi; \Delta_z)$. 
\end{enumerate}
\end{theorem}
...
\lefti \{ {\sf retn\_a}\,\lf{t_2} \}) \lefti \opsubst{D'}$
 \` (definition of $\opsubst{D}$)
\\
$\slst{\transop{\Sigma}}{\Psi}{\Gamma, [{!}({\sf eval\_a}\,\lf{t_1}
\lefti \{ {\sf retn\_a}\,\lf{t_2} \}) \lefti \opsubst{D'}]}{\Sp}{\susp{p^-_d}}$
 \` (given)
\\
...
\qquad $\slst{\transop{\Sigma}}{\Psi}{\Gamma,[\opbasic{A_1^+,\ldots,A_n^+}{t_{n+1}}{\sigma_0}]}{\Sp}{\susp{{\ocircle} C^+}}$ 
 \` ''\qquad\qquad~
\\
\qquad $T' :: (\Psi; \tackon{\opsubst{\Theta}}{C^+})
         \leadsto^*_{\transop{\Sigma}}
       (\Psi; \Gamma_z)$ 
 \` (given)
\\
