...
specification. However, as we expressed in
...
once we translate from a less expressive style to a more expressive
style, we can consider new modular extensions. The opportunity for
modular extension are what primarily distinguish the logical
correspondence we have presented from the work by Hannan and Miller
\cite{hannan92operational} and Ager \cite{ager04natural}. Both of
those papers translated natural semantics into a syntactic
specification of abstract machines that was modularly extensible to
the degree that concurrent \sls~specifications are.
...
Section~\ref{sec:trans-par}) can put natural semantics into logical
correspondence with either a sequential ordered abstract machine or a
parallel ordered abstract machine. In this section, we will consider
some other extensions: stateful features like mutable storage
...
A more modular semantics of parallel evaluation will be presented
in Section~\ref{sec:modular-parallelism}.
...
which forms the basis of imperitive algorithms. We will consider
