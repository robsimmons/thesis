...
continue to use the natural semantics specication of call-by-value
...
Abstract machine semantics are less prevelant than natural
...
$(\lambda x.e)\,\Box$ (an applicaiton frame with an evaluated fuction
...
arguments at either end constitue a logical correspondence between
...
of the form $\forall \overline{x}.\,p^+_\mtrue \lefti \{ A^+ \}$
and we have a single-step transition 
$(\Psi; \tackon{\Theta}{z{:}\susp{p^+_\mtrue}, y{:}\istrue{A^-}}) 
 \leadsto_{\Sigma} 
 (\Psi; \tackon{\Theta}{A^+})$
in an \sls~specification (witnessed by the step 
$(\tstep{\mkpat{A^+}}{y}{(\tforalll{\overline{t}}{\tappl{z}{\tnil}})})$), 
...
$(\Psi; \tackon{\Theta}{z{:}\susp{p^+_\mtrue}, y{:}\susp{\sf cont}}) 
 \leadsto_{\Sigma'} 
 (\Psi; \tackon{\Theta}{A^+})$
as well; this new transition is witnessed by the step
$(\tstep{\mkpat{A^+}}{\sf run\_cont}{(\tappl{(\tfuser{z}{y})}{\tnil})})$.
...
similarly unproblematic for ${\sf cont}$ to appear inside of a monadic
positive proposition, because after any transition it will either
continue to be inside of a monadic positive proposition or they will
have been released to live in the process state as suspended ordered
propositions. And by the same reasoning, we can replace propsitions
in the heads of signature clauses like this one:
\begin{align*}
& \Sigma, \\
& {\sf r} : {\sf a} \lefti \{ {\sf b} \fuse ({\sf c} \lefti \{ {\sf d} \fuse ({\sf e} \lefti \{ {\sf f} \}) \}) \}
\intertext{to produce a signature that looks like this:}
& \Sigma, \\
& {\sf cont1} : {\sf prop\,ord}, \\
& {\sf r1} : {\sf c} \fuse {\sf cont1} \lefti \{ {\sf d} \fuse ({\sf e} \lefti \{ {\sf f} \}) \}, \\
...
We can go further: even if $A^-$ contains free variables from $\Psi$,
it is always possible to define a type constructor $B^-$ with kind
$(\Pi x_1{:}\tau_1\ldots \Pi x_n{:}\tau_n.{\sf prop})$ 
such that $A^- = B^-\,t_1\ldots t_n$.
In this more general case, we can let
\begin{align*}
\Sigma'' = ~ & \Sigma,
\\    ~~ & {\sf cont} : 
       \Pi x_1{:}\tau_1\ldots \Pi x_n{:}\tau_n.\, {\sf prop\,ord},
\\    ~~ & {\sf run\_cont} : \forall x_1{:}\tau_1\ldots\forall x_n{:}\tau_n.\,
       {\sf cont}({\sf f}\,x_1\ldots\,x_n) \lefti B^-\,x_1\ldots x_n
\end{align*}
As long as 
