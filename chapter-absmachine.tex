...
the structure of backward chaining and then specfiy that
...
deductive computation as specificaiton in the concurrent fragment of
\sls. The first step in this process, representing the syntax
...
form $x{:}\susp{{\sf eval}\,\interp{e}}, \Delta$ or the form $x{:}\susp{{\sf
  retn}\,\interp{v}}, \Delta$. In the process of constructing 
a trace $x_e{:}\susp{{\sf eval}\,\interp{e_1\,e_2}}, \Delta
...
\item $\opbasic{{!}p^-_{ci},\ldots,{!}p^-_{cj},A^+_{j+1},\ldots,A^+_n}{t_{n+1}}{\sigma}$
  ...
  {\it (where
   ...
   and $\emptyset = FV(t_k^{\it in}) \cap (\overline{x_i} \cup \ldots \cup \overline{x_j})$ 
   for $i \leq k \leq j$)}
\end{itemize}
