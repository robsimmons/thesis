...
\item a pattern $P :: (\Psi; \tackon{\opsubst{\Theta}}{C^+}) \Longrightarrow_{\transop{\Sigma}} (\Psi; \tackon{\opsubst{\Theta}}{\Delta})$
\item and a trace of the form
...
\item $\opbasic{{!}p^-_{ci},\ldots,{!}p^-_{cj},A^+_{j+1},\ldots,A^+_n}{t_{n+1}}{\sigma_{i-1}}$
  \\
  $~ \qquad = \{ {\sf eval\_bi}\,\lf{({\sigma_{i-1}}t^{\it in}_i)}
                    \fuse  \ldots \fuse
