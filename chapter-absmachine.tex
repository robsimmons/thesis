...
above, so these two transformations combined with the adequacy
...
%
\intertext{Moving backwards, in the third step we have an expression $\lf{E_2} =
...
x{:}\tau_1.\,\tau_2(x) \rightarrow {\sf type}$), but we will not do so
here.
...
Each premise $A^+_i$ in this specific form either has the form
...
postfix 
structure that a transformed specifications impose on the
...
To allow the transformed programs to have parallelism, we again add a
...
& & {\sf a}, {\sf b}, {\sf c}, {\sf d} : {\sf prop}\,{\sf ord},\\
& & & {\sf cont1} : {\sf prop}\,{\sf ord},\\
& {\sf ruleA} : {\sf a} \lefti \{ {\sf b} \fuse {\downarrow}({\sf c} \lefti \{ {\sf d} \}) \},
& & {\sf ruleA} : {\sf a} \lefti \{ {\sf b} \fuse {\sf cont1} \},
\\
& \qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\quad  \Longrightarrow & & {\sf ruleA1} : {\sf c} \fuse {\sf cont1} \lefti \{ {\sf d} \},\\
& & & {\sf cont2} : {\sf prop}\,{\sf ord},\\
& & & {\sf cont3} : {\sf prop}\,{\sf ord},\\
& {\sf ruleB} : {\sf c} \lefti \{ {\downarrow}({\sf d} \lefti \{ {\downarrow}({\sf a} \fuse {\sf a} \lefti \{ {\sf b} \}) \}) \}
& & {\sf ruleB} : {\sf c} \lefti \{ {\sf cont2} \},
\\
& & & {\sf ruleB1} : {\sf d} \fuse {\sf cont2} \lefti \{ {\sf cont3} \},
\\
& & & {\sf ruleB2} : {\sf a} \fuse {\sf a} \fuse {\sf cont3} \lefti \{ {\sf b} \}
\end{align*}
...
The first arrow can be freely switched between left and right
...
As we discussed in Chapter~5, when we treat a natural semantics
specification as an inductive definitions, only the behavior of
...
implying that backtracking is not the correct choice.  Therefore,
...
figures, the standard evaluation judgment judgment $\obj{e \Downarrow
  v}$ is given the now-familiar evaluation and return predicates ${\sf
  eval}$ and ${\sf retn}$. The judgment $\obj{(v',e_z,x.e_s)
  \Downarrow^?v}$ is given evaluation predicate ${\sf eval}$, and {\it
...
original specification. Alternatively, if we take the model checking
...
applicability of generative invariants discussed in Chapter~9 to race
...
state, then proving the preservation of a generative invariant would
preclude the race condition.
...
expression is suspended. Once a value is needed, it is computed and
...
returns ${\sf issusp}\,l$ for the first time, the location $\lf{l}$
...
associated with a linear atomic proposition of the form ${\sf
  ...
  blackhole}\,\lf{l}$ proposition is needed for when an expression
tries to directly reference its own value in the process of computing
to a value will never evaluate to a value. In this example, such a
computation will end up stuck, but the comparison with recursive
suspensions suggests failing to terminate instead. A rule with the
...
but that presents a very different interface to the programmer, is
...
$\lf{{\sf lam}\,(\lambda x.\,e)}$. When a value of the form $\lf{{\sf
    lazylam}\,(\lambda x.\,e)}$ is returned to a frame $\lf{{\sf
    app1}\,e_2}$ in rule ${\sf ev/applazy}$ from
Figure~\ref{fig:ssos-by-need}, we immediately created a new expression
variable $\lf{x}$, suspended the argument $\lf{e_2}$, and schedule the
function body for evaluation. When a value of the form $\lf{{\sf
    evlam}\,(\lambda x.\,e)}$ is returned to a frame $\lf{{\sf
    app1}\,e_2}$ frame in rule ${\sf ev/appev1}$ in
...
we call ${\sf plus/carry}$ in Figure~\ref{fig:plus-1} does not include
...
something of the essence of an SSOS -- atomic transitions are
...
predicate ${\sf step}$, which assigns implies that the rules ${\sf
...
transformation is shown in Figure~\ref{fig:cbv-sos}. Note that the
...
structure of the ${\sf cont}\,\lf{f}$ propositions is analogous to the
