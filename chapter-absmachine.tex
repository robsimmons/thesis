...
\item If $D = {!}p^-_c \lefti D'$ and $p^-_c = {\sf ac}\,t_1\,t_2$, then 
  ...
  $T :: (\Psi; \opsubst{\Gamma}, x{:}{\sf eval\_a}\,t_1)
    \leadsto^*_{\transop{E}} (\Psi; \opsubst{\Gamma}, y{:}{\sf retn\_a}\,t_2)$
  \` (ind. hyp. (part 4) on $N$)
  \\
...
and then the we have a derivation of the form $\tlaml{\tbangl{x}{N}}$
...
\overline{s_i}$ that correspond to the correct variables) and a value
...
$T :: (\Psi; \tackon{\opsubst{\Theta}}{C^+}) \leadsto^*_{\transop{\Sigma}}
 (\Psi; \tackon{\opsubst{\Theta}}{y{:}{\sf retn\_a}\,(\sigma_nt_{n+1})})$. 
We will prove this
...
$T :: (\Psi; \tackon{\opsubst{\Theta}}{C^+}) \leadsto^*_{\transop{\Sigma}}
 (\Psi; \tackon{\opsubst{\Theta}}{y{:}{\sf retn\_a}\,(\sigma_nt_{n+1})})$. We proceed
...
\begin{align*}
  ...
  & \quad \leadsto^*_{\transop{\Sigma}} 
     (\Psi; \tackon{\opsubst{\Theta}}{y{:}\susp{{\sf retn\_a}\,(\sigma_nt_{n+1})}})
\end{align*}
...
specified,\footnote{Obviously, the restriction to propositions $A^-$
  ...
  more general transformation due our use of ordered logic.}  we can create
...
has the structure of a CBF grammar for describing well-formed
