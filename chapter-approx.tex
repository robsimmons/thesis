...
saturating logic programs, which makes derived specifications
executable.
...
instance, the methadology we describe only derives overapproximations
...
underapproximations (or {\it must-} analyses) that ensure that if a
...
our methadology by deriving two fundamental and rather different
overapproximation-based analyses: a context-insensitive control flow
analysis (Section~\ref{sec:0cfa}) and an alias analysis
(Section~\ref{sec:aliasanalysis}). Van Horn and Marison's related
``abstracting abstract machines'' methadology, described in
...
programming on purely-persistent speficiations and like simple
...
that the specificiation cannot be run as a saturating logic program,
and being able to interpret specifications as saturating logic
programs is a prequesiate for applying the meta-approximation theorem
...
derivable facts is a simple and sufficient critera for showing that a
...
possibilities.\footnote{This statement assumes that both of the
  ...
  $\obj{x}$ evaluates to $\obj{(\lambda y\ldots)}$, for instnace, if
...
substitutie an appropriate concrete term for $\lf{x}$ that is uniquely
...
with an expresion (the tag) representing the site where it was bound.)
...
$\lf{E({\sf var}(\lambda x.E\,x))}$ -- a subterm of $\lf E$.  The new
...
need recall the question that we intend to answer with
control flow analysis. 


The primary question that such a flow analysis
...
\subsection{Acceptability as a control flow analysis}
