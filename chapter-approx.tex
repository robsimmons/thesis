...
for ordered and linear logical specifications.\robnote{If I want
...
specification of a monadic functional language with Lisp-like mutable cons
cells and derive an interprocedural alias analysis. The resulting approximation
bears a strong resemblance to the object-oriented alias analysis presented
as a logic program in \cite[Chapter 12.4]{aho07compilers}.
...
There are five commands in our example language: the command
$\interp{\lambda x.e} = {\sf fun}(\lambda x.\interp{e})$ generates the
function value $\interp{\lambda x.e} = {\sf lam}(\lambda
x.\interp{e})$, and the commenad $\interp{f\,x} = {\sf call}\,f\,x$
expects a function value to be bound to the variable $f$ and calls it
with the value bound to variable $x$. The ordered abstract machine for
this specificaiton is given in Figure~\ref{fig:ssos-monadic}; note
that the fact that ${\sf variable}$ and ${\sf exp}$ are distinct types
makes the environment semantics quite natural: the predicate ${\sf
  bind}$ has kind ${\sf variable} \rightarrow {\sf value} \rightarrow
{\sf prop}\,{\sf pers}$.
