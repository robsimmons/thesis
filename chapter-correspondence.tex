...
$T :: (\Psi; \tackon{\Theta}{C^+}) \leadsto^*_{\transop{\Sigma}}
 (\Psi; \tackon{\Theta}{y{:}{\sf retn\_a}\,})$. We will prove this
...
$T :: (\Psi; \tackon{\Theta}{C^+}) \leadsto^*_{\transop{\Sigma}}
 (\Psi; \tackon{\Theta}{y{:}{\sf retn\_a}\,s})$. We proceed
...
\item $\opbasic{}{t_{n+1}}{\sigma_n} = \{ {\sf retn\_a}\,(\sigma_{n}{t_{n+1}}) \}$
  ...
  $(\Psi; \tackon{\Theta}{{\sf retn\_a}\,(\sigma_{n}{t_{n+1}})})$ decomposes
  to $(\Psi; 
  \tackon{\Theta}{y{:}{\susp{{\sf retn\_a}\,(\sigma_{n}{t_{n+1}})}}})$
...
\item $\opbasic{{!}{\sf a}\,t^{\it in}_n\,t_{n+1}}{t_{n+1}}{\sigma_{n-1}} 
  ...
  $(\Psi; \tackon{\Theta}{y{:}{\sf retn\_a}\,s})$. Because 
...
\item $\opbasic{p^+_\mpers,A^+_{i+1},\ldots,A^+_n}{t_{n+1}}{\sigma_{i-1}} 
  ...
  \\
  $T :: (\Psi; \tackon{\Theta}{C^+}) \leadsto^*_{\transop{\Sigma}}
   (\Psi; \tackon{\Theta}{y{:}{\sf retn\_a}\,s})$
  \` (by inner ind. hyp.)
  \\
...
\end{align*}
  ...
  \` (given) 
  \\
  $\slst{\Sigma}{\Psi}{\Gamma}{\tbangr{N_k}}{[{!}{{\sf bk}\,({\sigma_{i-1}}t_k^{\it in})\,({\sigma_j}t_k^{\it out})}]}$
  \` (side translation of ) 
  \\
  \end{tabbing}
...
\item $\opbasic{{!}G,A^+_{i+1},\ldots,A^+_n}{t_{n+1}}{\sigma} = \forall
  ...
  \smallskip
  We are given a value 
  $\slst{\Sigma}{\Psi}{\Gamma}{\tbangr{N}}{[{!}\sigma_i{G}]}$; observe that
  $\sigma_i = (\sigma_{i-1}, \overline{s_i}/\overline{x_i})$.

  \smallskip
  We get $\Sp'$ and $T$ directly from the induction hypothesis.
  The necessary result has the form 
  $\tforalll{\overline{s_i}}{\tappl{\tbangr{N'}}{\Sp'}}$;
  an appropriate number of instances of $\forall_L$ followed by
  ${\lefti}_L$ on $N'$ and $\Sp'$. 

\end{itemize}

This inner induction concludes the proof.
\end{proof}
