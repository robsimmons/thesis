...
\item The {\it natural semantics}, or big-step operational semantics,
  is an existing well-known specification style that is convenient for
  the specification of pure and deterministic programming
  languages. 
...
into destination-passing semantics, by fully automatic and
...
our logical correspondence and the {\it functional correspondence}
...
evaluation, and recoverable failure.
...
\cite{ager04natural} have also proposed the idea of operationalization
...
will either remain defined by deductive proofs (we write these as
...
\item $\transbasic{\Sigma, {\sf r} : C} = \transbasic{\Sigma}, {\sf r}
  ...
  \ldots, A^+_n \rrbracket (t_{n+1}, {\sf id})$ \\ {\it (where $C$ is
    ...
    \overline{x_n}.\, A^+_n \righti \ldots \righti A^+_1 \righti {\sf
      a}\,t_{0}\,t_{n+1}$)}
...
: \forall{x}.\,\forall{y}.\,{!}{\sf a}\,x\,y \righti {\sf a}\,({\sf
...
t_{n+1}$ unless these functions are {\it fully general}, meaning that
of their free variables are in $\overline{x_n}$ (and therefore not
fixed by the input or any other premises) . If we have a function
