...
the introduction, where the judgment $\obj{e \Downarrow v}$ expresses that
the $e$ evaluates to the value $\obj{v}$:
\[
\infer[{\sf ev/lam}]
...
specify whether $\obj{e_1}$ or $\obj{e_2}$ 
should be evaluated in some particular
...
such that $\obj{e \Downarrow v}$) from failure (concluding finitely that
there is no $\obj{v}$ such that $\obj{e \Downarrow v}$).
...
\sls~prototype;\robnote{Finish implement destination-adding or don't say
  that you did.} approximation, on the other hand,
...
of the context
matching $A^+$ appears to its right, so  $\left(x{:}\susp{\sf p1(\lf{c})}, ~
y{:}\istrue{\left( {\sf p1(\lf{c})} \lefti \{ {\sf p2(\lf{c})} \} \right)}\right)
\leadsto \left(z{:}\susp{\sf p2(\lf{c})}\right)$, whereas 
$\left(y{:}\istrue{\left( {\sf p1(\lf{c})} \lefti \{ {\sf p2(\lf{c})} \} \right)},~
x{:}\susp{\sf p1(\lf{c})}\right)
...
in the \sls~prototype.\robnote{Implement destination-adding or backtrack
on this statement.} In such a specification, a change from
