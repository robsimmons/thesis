...
manipulations to a stack-like structure structure.  The push-down
...
ordered context as well, as we saw from the SSOS specifications for
...
Ordered logic does
allow us to express any (flat, concurrent) specifications that 
we could not express with linear logic alone. 
...
{\it destination-adding}, from specifications in ordered logic
to specifications in linear logic. This
...
of the form $\forall \overline{x}. S_1 \lefti \{ S_2 \}$. As discussed
in Section~\ref{sec:defunc-uncurry}, the difference between $\righti$
and $\lefti$ is irrelevant in this situation.\footnote{The monad $\{
  S_2 \}$ did not actually appear in \cite{simmons11logical}, and the
  presentation took polarity into account but was not explicitly
  polarized. We are justified in reading the lax modality back in by
  the erasure arguments discussed in
  Section~\ref{sec:perm-fragments}.} The restriction to flat
...
translation is given is sufficient for our purposes. Therefore, we
...
\lf x.\,A^+ \lefti B^-$). The result is an specification, equivalent
...
\begin{proof}
  This proof is given in detail as \cite[Theorem 2]{simmons11logical};
  we will sketch the outlines here. The transformation $\llbracket
  \Psi; \Delta \rrbracket$ on process states introduces a leftmost
  destination $d_L$ and a rightmost destination $d_R$ -- these two
  will be the same if the context contains no destinations. Let
  $\llbracket \Psi; \Delta \rrbracket^{d_L}_{d_R}$ indicate that the
  leftmost and rightmost destinations resulting from the translation
  were $d_L$ and $d_R$, respectively.

First, we need to prove a number of theorems by induction over right-focused
derivations (Lemma 1, \cite{simmons11logical}):
\begin{itemize}
\item If  $\llbracket \Psi; \Delta \rrbracket^{d_L}_{d_R} \vdash_\Sigma [\llbracket S \rrbracket^{d_L}_{d_R}]$
\item If $\Psi; \Delta \vdash_\Sigma [S]$ and $S$ contains no ordered
  atomic propositions, then $\restrictto{\Delta}{\meph}$.
\end{itemize}
The first lemma, relates destination passing in right focus. It is 
easy to prove by induction that $\Psi; \Delta \vdash_\Sigma [S]$
implies 

The
transformation $\llbracket \Psi; \Delta \rrbracket$ on process states
introduces a leftmost destination $d_L$ and a rightmost destination
$d_R$; these two will be the same if the context contains no
destinations.
 
proving that $\llbracket \Psi; \Delta \rrbracket$
\end{proof}
...
\subsection{Alternate semantics for parallelism and failure}
...
substituted it for the bound variable $\lf{c}$ in $\obj{e}$). 
...
Futures use of destinations to create new and potentially disconnected
...
connected not by destinations by the variable $\lf x$, which the
...
therefore stand for continuations in much the same way that introduced
