...
destination-adding transformation was to exposes information about the
control structure of computations, which could then be harnessed by a
the program abstraction methodology described in the next chapter to
derive program analyses. The exposure of control structure also allows
us to modularly extend specifications with additional features
relating to control. The semantics of parallelism and failure
presented in Section~\ref{sec:modular-parallelism} is new, the
semantics of futures (Section~\ref{sec:dest-futures}) and
syncronization (Section~\ref{sec:dest-synch}) are a variant on the
specifications first presented in the CLF tech report
\cite{cervesato02concurrent}, and the semantics of first-class
continuations presented in
...
In Section~\ref{sec:failure}, we discussed how the ordered abstract
machines
