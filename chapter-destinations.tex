...
\begin{proof}
  ...
  First, we need to prove that a right-focused proof of $\Delta
  \vdash_{\Sigma} S$ implies that there is an analogous proof of
  $\llbracket \Delta \rrbracket \vdash_{{\it Dest}(\Sigma)} [
  \llbracket S \rrbracket^{\lf{d_L}}_{\lf{d_R}} ]$, and conversely
  that if we can prove $\Delta \vdash_{{\it Dest}(\Sigma)} [\llbracket
  S \rrbracket^{\lf{d_L}}_{\lf{d_R}}]$ in right focus under then
  linear translation, then it is possible to reconstruct an ordered
  context $\Delta'$ such that $\llbracket \Delta'
  \rrbracket^{\lf{d_L}}_{\lf{d_R}} = \Delta$ and $\Delta'
  ...
  it that it is possible to reconstruct the ordered context from the
  ...
  The theorem then follows from directly from these two lemmas.  There
  ...
  value used to prove $S_1$ (the first set of lemmas), and then begin
  inverting with the positive proposition $S_2$ in the context (the
  second set of lemmas).
\end{proof}
