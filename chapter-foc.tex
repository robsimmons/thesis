...
over. The presentation of focusing for intuitionstic linear logic in
this section most closely resembles Chaudhiri's focused intuitionstic
linear logic \cite{chaudhuri06focused} and my presentation of
polarized intuitionstic persistent logic
...
$\with$ in full propositional linear logic) and call the synchronous
connectives {\it positive} ($\zero$, $\oplus$, $\one$, and $\otimes$
in full propositional linear logic). Each atomic proposition,
furthermore, must be assigned to be either positive or negative,
though this assignment can be arbitrary. At this point, there is an
important choice to make. One way forward is to treat positive and
negative propositions as a syntactic refinements of all propositions,
in which case we end up focusing a standard intuitionstic linear
logic. The other way forward is to treat positive and negative
propositions as distinct syntactic classes $A^+$ and $A^-$ with
syntactic inclusions between them. In this second case, we end up
focusing a {\it polarized} linear logic.  These inclusions are
traditionally called {\it shifts}. The positive proposition
${\downarrow}A^-$, pronounced ``downshift $A$,'' has a subterm that is
a negative proposition; the negative proposition ${\uparrow}A^+$,
pronounced ``upshift $A$,'' has a subterm that is a positive
proposition.
...
derivation of $A^-$.\footnote{It would have been equally valid to say
  $(A^+)^\circ$ is derivable if and only if $A^+$ is derivable.}
However, most proofs of the correctness of focusing prove a slightly
weaker property. Every proposition in linear logic has an obvious
polarized analouge with a minimal number of shifts, given by the two
functions $A^\oplus$ and $A^\ominus$ in Figure~\ref{fig:lin-shift}, so
almost all proofs of correctness of focusing establish that there is
an unfocused proof of $A$ if and only if there is a focused proof of
$A^\ominus$.\footnote{The two exceptions are Zeilberger completeness
  ...
  proof in intuitionstic persistent logic \cite{simmons11structural}.}
The weaker formulation is sufficient for our current purposes, so we
will discuss the weaker property of polarization-based correctness,
not erasure-based correctness.
...
The views are quite similar -- the primary difference 

{\it negative} and write them as
$A^-$, and call the synchronous connectives {\it positive} and write
them as $A^+$. At this point, there is a choice to make.




It is possible to
treat the positive and negative connectives as syntactically distinct

 due to the
syntactic differentiation of these classes, 

 In the fragment of
linear logic presented earlier, the propositions end up classified
like this:
\begin{align*}
A^+ & ::= p^+ \mid {\downarrow}A^- \mid {!}A^- \mid \one \mid A^+ \otimes B^+\\
A^- & ::= p^- \mid {\uparrow}A^+ \mid A^+ \lolli B^-
\end{align*}
There are two issues to note. First, the premise of linear implication $A \lolli
B$ is positive, even though the connective itself is negative. This is
because, in order to stay in focus. 
