...
it is critical that ${!}_R$ lose focus in its premise.\robnote{I can't
remember the snappy counterexample right now.} 
\[
\infer[{\pbang}_R]
{\mildseq{\Gamma}{\cdot}{[{\pbang}p^+]}}
{\mildseq{\Gamma}{\cdot}{[p^+]}}
...
rule, on the other hand, is invertable and so naturally negative,
...
context. This is the reason for arguing that ${\pbang}$ captures ony
...
\cite{reed09judgmental}. It has characterized much of my own work on
focused logics with the atom and bang optimizations
\cite{pfenning09substructural,simmons09weak,simmons11logical}.
Adjoint logic, in Benton and Wadler's formulation, generalizes both
linear logic and lax logic as sublanguages of a common logic, whose
...
persistent context $\Gamma$ and as the succeedents of sequents
$\pseq{\Gamma}{X}$, and linear propositions $A$ are contained in the
linear context $\Delta$ and as the succeedents of sequents
...
In previous work, I have generally required that focused substructural
logics enforce {\it separation}. In our current development, the
separation condition says that each atomic proposition either {\it
  always} appears as a subformula of ${\pbang}p^+$ or it {\it never}
appears as a subformula of ${\pbang}p^+$. If we enforce separation,
then it is easy to encode our optimized focused logic as a fragment of
a naturally focused adjoint logic. Positive atomic propositions that
are always associated with ${\pbang}$ are encoded as {\it persistent}
positive atomic propositions $x^+$, and positive atomic propositions
that are never associated with ${\pbang}$ are encoded as {\it linear}
positive atomic propositions $a^+$. The proposition ${\pbang}p^+$ can
then be translated as $F x^+$, where $x^+$ is the translation of $p^+$
as a persistent positive atomic proposition.

This gives one solution to why, in linear logic, we can't naturally
substitute positive propsitions for positive atomic propositions when
...
the atom and bang optimziations points towards the existance of more
