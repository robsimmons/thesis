...
aspects of state and state transitions in a natural way. In
Section~\ref{sec:linlogicalframeworks}, we discuss why the normal
presentation of linear logic is insufficient as {\it logical
  frameworks} -- proofs in linear logic suffice to establish the
...
that addresses this first problem in Section~???; we also explain why
the resulting system is less natural and discuss the implications of
that observation. The second deficiency is that focusing gives us a
highly sequential transition system, whereas our goal is to give an
interpretation of concurrent transition systems. In Section~???, we
describe {\it concurrent equality}, an equivalence relation on focused
derivations introduced by Watkins et. al. to solve this problem, and
also consider its limitiations in a fully focused setting.
...
from Section~???. In Chapter 4 we will carve out a fragment of this
logic that forms the basis of the logical framework of substructural
logical specifications; this framework will also incorporate the
concurrent equality sketched out in Section~???. The purpose of this
chapter is to consider the issues that arise in our rich substructural
setting within the comparatively simple setting of intuitionstic
linear logic.
