...
incoroprating the structure that we are interested in into a focused
...
fragment:\footnote{We will use the sequent form
  ...
  faithful intuitionstic analouge of Andreoli's system. This is, of
...
syntatically valid proposition; the only obvious candidate seems to be
...
presentation of intuitionstic linear logic.
...
postivie proposition will be based on an analysis of {\it
  permeability}.  Permeability in classical presentations of linear
logic dates back to Girard's LU \cite{girard93unity}.\robnote{Check
  with Dale: earlier citations?} In this section, we will motivate
permeable atomic propositions in intuitionstic linear logic by first
considering a new identity expansion principle that only applies to
permeable propositions, which are a syntatic refinement of the
positive propositions.\footnote{Permeable {\it negative} propositions
  ...
  intuitionstic linear logic means that it is reasonable to consider
  permeability exculsively as a property of positive propositions.}
...
Another pattern
that we might want to capture with something like identity expansion
is the situation where we are trying to prove the sequent
$\mildseq{\Gamma}{\Delta}{1}$ and we know, by the syntactic structure
of $\Delta$, that inversion will empty the context; one instance of
this pattern is the sequent $\mildseq{\Gamma} {{!}A \otimes {!}B
  \otimes {!}C \otimes {!}D} {\one}$. Positive propositions 
like ${!}A \otimes {!}B \otimes {!}C \otimes {!}D$ that,
upon decomposition, remove themselves from the linear context altogether
are called {\it permeable}, as in a certain sense 
they might as well be in the persistent context. It is possible to
precisely characterize the syntactic refinement of the 
...
us a new focal substitution principle ${\it subst}_p^+$. (This was
