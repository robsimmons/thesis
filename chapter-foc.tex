...
refinements of the set of all propositions. This will lead to the
development focused presentation of a standard intuitionistic linear
logic with the connections and propositions that we have already
considered. The other way forward is to treat positive and negative
propositions as distinct syntactic classes $A^+$ and $A^-$ with
explicit inclusions, called {\it shifts}, between them. In this second
case, we end up focusing a {\it polarized} linear logic.  The positive
proposition ${\downarrow}A^-$, pronounced ``downshift $A$'' or ``down
$A$,'' has a subterm that is a negative proposition; the negative
proposition ${\uparrow}A^+$, pronounced ``upshift $A$'' or ``up $A$,''
has a subterm that is a positive proposition.
\begin{align*}
A^+ & ::= p^+ \mid {\downarrow}A^- \mid {!}A^- \mid \one \mid A^+ \tensor B^+\\
A^- & ::= p^- \mid {\uparrow}A^+ \mid A^+ \lolli B^-
\end{align*}

Polarized logics are independently interesting as objects of
investigation: for instance, the Curry-Howard interpretation of
polarised persistent logic has been independently studied by Levy as
Call-by-Push-Value \cite{levy04call}.  However, the independent study
of polarized logic is not our current goal; rather, we want merely to
characterize the structure of focused proofs and show that every
derivation in linear logic can be turned to a focused proof in linear
logic. Polarization does not explicitly help us achieve this goal, but
the use of polarized logic simplifies the proof of identity expansion
in Section~\ref{sec:linindentity} and the proof of completeness in
Section~\ref{sec:lincorrectness}, so we will describe focusing for a
polarized linear logic with shifts, starting with the following
grammar of polarized propositions, positive and negative:
...
$\underline{\Delta}$ and succedents $\underline{U}$ allows them to
...
succedent $U$ is either a positive proposition $A^+$ or a suspended
...
incorporated a focused version of the ${\it init}$ rule that also
applies only to atomic propositions. This treatment is not incorrect
and is obviously analogous to the ${\it init}$ rule from the unfocused
system. Nevertheless, I believe this to be a design error, and it is
one that has historically made it enormously (and unnecessarily)
difficult to prove the identity theorem for focused systems. This
presentation uses {\it suspensions}: suspended positive propositions
$\langle A^+ \rangle$ only appear in the linear context $\Delta$, and
suspended negative propositions $\langle A^- \rangle$ only appear as
succedents. They are treated as stable (we never break down a
suspended proposition) and are only used to immediately prove a
proposition in focus with one of the identity rules ${\it id}^+$ or
${\it id}^-$. The rules ${\it id}^+$ and ${\it id}^-$ are more general
focused versions of the unfocused ${\it init}$ rule.\footnote{As we
  discuss below in Section~\ref{sec:linindentity}, this extra
  generality does not influence the structure of proofs because
  suspended propositions can only be introduced into the context or
  the succedent by the $\eta^+$ and $\eta^-$ rules, and those rules
  {\it are} restricted to atomic propositions.}
...
\begin{proof}
  ...
  ${\downarrow}{\uparrow}_L$, both of which are derived below, to get
  rid of all the extra shifts.  (Zero or more instances of a rule are
...
interferes with the structure of cut admissibility, the bang
