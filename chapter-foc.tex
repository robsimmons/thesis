...
proposition up through the second occurance of a shift of exponential)
...
a^-$ or if $\Delta = \cdot$ and $a^- \in \Gamma$, then $a^-$ is a {\it
  pseudo-positive} atomic proposition. Proving the succeedent
$\susp{a^-}$ where $a^-$ is pseudo-positive is functionally very
similar to proving the succeedent $\susp{[a^+]}$ for a positive atomic
...
Note that the ${!}'_R$ rule is not invertable,
