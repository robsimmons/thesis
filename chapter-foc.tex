...
contain only atomic suspended propositions. Thereforem
...
\begin{proof}
  ...
  lemams use two derived forms, ${\downarrow}{\uparrow}_R$ and
...
inversion phase echo the discussion of nondeteterminism in 
...
expressed this admissibile rule:
\[
\infer-[{\it cut}]
...
on the other hand, all scale linearally with the number of connectives
...
currently possess in order to enact a transition. The 2010 health care
reform law's calorie labeling requireiments say that, in the future,
we can exchange six bucks for a soup and salad at Panera, but only if
we know how many caolries are in the meal. The six bucks, soup, and
salad remain ephemerial resources like $q$ and $r$, but the calorie
count is persistent. It is mathematical knowledge, which obviously is
not consumed by the transation.

My justification for presenting
Chaudhuri's system as the canonical focusing system for linear logic
in Section~\ref{sec:foclinlog} is because it truly treates the focused
sequent calculus {\it as a logic}. Internal soundness and 
completeness properties can are established by the cut admissibility
...
sequent calculi for linear logic. Andreoli's atom optimization makes
cut admissibility for the focused system complicated and non-structural, 
and, for systems based on adjoint logic, it is difficult or impossible to
define a well-behaved erasure function in such a way that 
polarization strategies -- partial inverses of erasure -- exist. Furthermore,
both modifications weaken our ability to read atomic proposition as stand-ins
for regular propositions in linear logic. 
...
immediately blurs, as in this (intuitionstic) 
...
Identity expansion let us see the first cloud on the horizion for the 
suitability of the atom optimization. Previously, when we substituted
a positive proposition for an atomic proposition, the proof's structure
remained fundamentally unchanged -- instances of the $\eta^+_{p^+}$ rule
on the replaced atomic proposition turned
into admissibile instances of the general identity expansion rule 
$\eta^+_{A^+}$. We first have to explain what ${\pbang}p^+$ even becomes
when we substitute $A^+$ for $p^+$; the most obvious candidate is
...
of this pattern arises when we try to establish cut admissibility for the
logic with ${\pbang}p^+$. 
...
the cut in Figure~\ref{fig:bad-cut}. We cannot immediately 
...
to make $n$ calls to part 3, of cut admissibility
followed by $n$ uses of $\eta^+$, where
$n$ is determined by the inductive call to part {\it 6a} or {\it 6b}. 

The calls to part 3 are justified by the induction metric: the principal
cut formula $p^+$ stays the same and the part number gets smaller.
\end{proof}
...
This extra step, in the context of a more complex setting like the 
ordered linear lax logic considered in Chapter 3, is a significant
