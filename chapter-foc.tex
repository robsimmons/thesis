...
\begin{proof}
  ...
  left commutative cuts (where the first given derivation gets
...
defined on focused sequents 
$\mildseq{\Gamma}{\underline{\Delta}}{\underline{U}}$ when
$\underline{\Delta}$ and $\underline{U}$ 
contain only atomic suspended propositions. Therefore
Theorems~\ref{thm:linfocsound}~and~\ref{thm:linfoccomplete}, like
the cut admissibility theorem, implicitly have 
as an extra condition that $\underline{\Delta}$ and $\underline{U}$
...
stable,\footnote{Frank Pfenning's 
  ...
  but the trade-off is that he must use a more general statement of 
  cut admissibility that also makes fewer
  stability demands.} 
...
This unfocused cut rule is
independently interesting, and in any case we should expect it to
hold. Pfenning has noted\footnote{Personal communication.} that by
proving this simpler cut property, we can prove a simpler class of
unfocused admissibility lemmas on unstable inversion sequents. The
unfocused admissibility of rules like ${!}_R$, ${\lolli}_R$, and
${\otimes}_L$, which required the use of doubly-shifted side proofs in 
our proof, is trivial in this alternate presentation.
Unfortunately, the proof of unfocused cut is 
tedious and long. The size of all known
proofs of the admissibility of unfocused cut 
grows quadratically with the number of connectives and
rules included in the logic. The proofs of identity expansion,
cut admissibility, soundness of focusing, and completeness of focusing,
on the other hand, all scale linearly with the number of connectives
...
for our purposes in later chapters.
...
the persistent and linear contexts and that permable atoms can be introduced
to stand for 
\end{itemize}
...
\paragraph{Soundness and completenss of focusing}
...
establised with ${\it copy}$ and ${\it init}$ in the unfocused system:
\[
\infer[{\it copy}]
...
Completeness reqires that we prove two new unfocused admissibility rules
dealing with ${\pbang}$,
...
{\it extenally} sensible relative to normal linear logic, because
...
influence the {\it existance} of proofs, not just the structure of
proofs.\footnote{Both the note of Girard and the paper of Melli{\`e}s
  ...
  proposition $A$ that is classically but not intuitionstically true,
  we know that the double negation $\neg\neg A$ will be true in
  intuitionstic logic! This view of shifts as negations seems rather
...
the soundness theorem is straightforward. For completeness, we must 
...
problematic: it requires us to prove a new cut principle,
