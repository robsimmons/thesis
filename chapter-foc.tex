...
In our earlier discussion of hereditatry substitution and canonical
forms, we mentioned that the normalizaiton theorem provided by
...
On the other hand, when we perform hereditary substitution to obtain a
Canonical LF term from an arbitrary non-canonical LF term, we gain
{\it no guarantees} about the relationship between the non-canonical
LF term and the Canonical LF term. The statment of the theorem does
not preclude taking a $\beta$-normal, $\eta$-long LF term (like
$\lambda x. \lambda y. x$ of type $A \rightarrow A \rightarrow A$)
into a structurally different Canonical LF term (like $\lambda
x. \lambda y. y$, which also has type $A \rightarrow A \rightarrow
A$). It is possible to gain such a guarantee for LF, as Martens and
Crary have shown in unpublished work \cite{martens11mechanizing}, but
...
proof by way of the constructive content of
Theorem~\ref{thm:linfoccomplete}, only to obtain a proof that was not
identital or even related. This is not at all a merely hypothetical
concern. In the structural focalization development, the proofs are
mechanized and so their constructive content can easily be obtained.
Our example in Section~\ref{sec:linconcurrenteq} of a proposition
with two multifocusing-equivalent proofs that are not concurrently
equal is also representable in intuitionstic logic as the proposition
   $a^+ \supset 
...
The proposition in Section~\ref{sec:linconcurrenteq} demonstrating
the difference between concurrent equality and multifocusing. 
The mechanization of the structural focalization development.
...
While multifocusing does correspond to a natural
notion of proof equivalence -- the equivalnce relation that includes
all permissible permutations of individual rules -- we did not give.
