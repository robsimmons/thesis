...
judgments.\footnote{Treating these extra conditions as a side
  ...
  write them constantly in proofs.}  Therefore, from this point on, we
...
and $\eta^-$ rules are restricted to negative propositions, and there
is no other way for suspended propositions to be introduced into the
context with rules, so it would seem be natural to restrict ${\it
  id}^+$ to atomic propositions as well. To explain why this seemingly
natural choice is the wrong one, we will turn our attention to
...
inference rule beyond the outermost two polarity shifts. 
\[
