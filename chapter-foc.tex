...
propositions. In this section, I will discuss the difference between
...
${\pbang}p^+$, we can conclude by case analysis\footnote{Or rule inversion,
in the sence.}
that the first given derivation 
must prove $p^+$ in focus using the ${\it id}^+$ rule. This, in turn,
means that $\langle p^+ \rangle$ must already appear in $\Gamma$,
so the cut reduces to an admissible instance of contraction.
\[
\infer-[{\it cut}(1)]
{\mildseq{\Gamma, \langle p^+ \rangle}{\Delta}{U}}
{\infer[{\pbang}_R]
...
{\mildseq{\Gamma, \langle p^+ \rangle, \langle p^+ \rangle}{\Delta}{U}}
\]
Therefore, we no longer need the extra parts {\it 6a - 6c} of cut 
...
tensor logic \cite{mellies10resource}, as well as to the logic of
Girard that introduced the ${\uparrow}A^+$ and ${\downarrow}A^-$
...
subformula -- {\it in lieu} of the focus-interrupting ${!}A^-$
connective.  Both presentations also have the prominent caveat that
...
proofs.\footnote{Both the note of Girard and the paper of Melli{\`e}s
  ...
  viewpoints \cite{zeilberger08unity}.} In doing so, this view of
logic threatens the valuable intuition that the resource
interpretation of linear logic provides.
