...
framework of substructural logical specifications. The purpose of this
chapter is to consider the issues that will arise in our richer
substructural setting within a comparatively simple setting.
...
number of $A$ resources, including zero. A sandwich shop that allows
six dollars to be exchanged for both soup and a sandwich any number
of times could be represented as the resource
...
intuitionistic linear logic.\footnote{In this chapter we will mostly
  ...
  x.A$. The ``why not'' modality $\mbox{?}A$ from classical linear
  ...
  not consider that modality in this thesis.} It corresponds most
...
An important point, which we will return to in
...
If consider this discipline applied to our robot example where all
...
{\infer[{\it id}]
 ...
 {\vdots}}}
\]
The trace (that is, the bottom sequent and unproved top sequents) of a
...
presentation for intuitionistic linear logic with the connections and
...
In this thesis, we will be interested only in the structure focused
proofs, which correspond to the polarization strategy given by
...
restriction, whenever we consider a focused sequent
...
Figure~\ref{fig:linear}), because the general ${\it id}$ rule, which
concludes $\seq{\Gamma}{A}{A}$ for all propositions $A$, is admissible
just as the ${\it cut}$ rule is admissible. But while the ${\it cut}$
rule can be eliminated completely, the atomic ${\it id}$ rule must
remain. This is related to the logical interpretation of atomic
propositions is that they are stand-ins for unknown propositions.  All
...
unable to break down atomic propositions any further, and this is why
the ${\it id}$ rule is necessary at atomic propositions.  If we
substitute a concrete proposition for some atomic proposition, the
structure of the proof stays exactly the same, except that instances
of initial sequents become admissible instances of the identity
theorem.
...
containing the suspended proposition. In the Structural Focalization
development \cite{simmons11structural}, I discuss how, if we encode
...
need to make a critical shift of perspective from
...
${\it id}$ puts an end to this process when an atomic proposition is
...
\begin{proof}
  ...
  {\small \noindent($\Delta$ are stable by assumption) \[
  \infer-[\it cut (1)]
  ...
  {\small \noindent ($\Delta$ is stable by assumption, $\Delta'$ and
    $U$ are stable by the side condition on rule ${\it focus}_L$)
  \[
  \infer-[\it cut (4)]
  {\mildseq{\Gamma}{\Delta', \Delta}{U}}
  {\deduce{\mildseq{\Gamma}{\Delta}{A^-}}{\mathcal D}
...
There is a notable asymmetry between part 1 of the theorem, which does
...
succedents, which is done in Figure~\ref{fig:lin-shift-ctx}. Note
...
\begin{proof}
  ...
  $C_1^-$). Then, by case analysis on $C_1^-$, it can only be the case
  that $C_1^- = {\uparrow}C_2^+$. But then $C_2^+$ can be either
  ${!}B_2^-$ or ${\downarrow}C_3^-$; in the latter case $C^+ =
  ...
  and $A = (A^-)^\circ$. Depending on the case, we then repeatedly
  apply either the ${\uparrow}{\downarrow}_R$ or the
  ${\downarrow}{\uparrow}_L$, both of which are derived below, to 
  eliminate all the extra shifts.  (Zero or more instances of a rule are
  indicated by a double-ruled inference rule.)
...
rest of the proof.  This property is already seems necessary to prove
...
possible to prove a more general completeness theorem --
Theorem~\ref{thm:linfoccomplete} as presented only applies to stable
sequents. The cases of this theorem dealing with the rules ${!}_R$,
${\lolli}_R$, and ${\otimes}_L$, which required the use of
doubly-shifted side derivations in our proof, is trivial in this
...
lineraly in this way: all known proofs grow quadratically with the
...
sufficient for our purposes here and in later chapters. As such, we
will not prove confluence, though doing so is a straightforward
exercise.
...
(Theorem~\ref{thm:linfoccomplete}); we discussed but did not mention
the unfocused cut admissibility rule. The second idea is that the
proof of a stable sequent must end with one of three rules. Once we
know whether the last rule is
\begin{itemize}
...
\end{itemize} (it must be one of these three), then the bipole
structure of the proposition we focus on is all we need to know in
order to understand the structure of the proof up to the next
...
resources are distributed between different branches of the context.
If we also consider additive connectives, we can identify some number
of synthetic rules for each right focus, left focus, or copy. This may be 
zero, as there's no way to successfully right focus on a proposition like
$\zero \otimes {\uparrow}{\downarrow}A^+$, or it may be more than one:
there are three ways to successfully right focus on the proposition
$a^+ \oplus b^+ \oplus c^+$.
...
persistent. It is mathematical and scientific knowledge, 
a resource that is not consumed by the transition.
...
\]
However, if we can prove a general theorem that the sequent
$\mildseq{\Gamma}{\Delta}{\susp{a^-}}$ can only be proven if $\Delta =
a^-$ or if $\Delta = \cdot$ and $a^- \in \Gamma$, then the persistent
proposition ${\downarrow}a^- \lolli {\downarrow}b^- \lolli c^-$ can be
associated with these two synthetic inference rule-like rules:
\[
\infer
...
\]
The machinery of lax logic introduced in Chapter~3, along with the
separation of concurrent and deductive proofs introduced in Chapter~4,
are what make it possible to reason about negative propositions like
$a^-$ that act like positive propositions in practice. 
...
In our attempts to motivate a notion of persistent positive
propositions, we have now surveyed Andreoli's atom optimization that
interferes with the structure of cut admissibility, the exponential
optimization that lacks a good interpretation in unfocused linear
logic, and an adjoint formulation of linear logic that introduces
persistent positive propositions as members of a syntactic class $X$
of persistent propositions that usually lies hidden in between the two
right-synchronous and right-asynchronous (that is, positive and
negative) halves of the ${!}$ connective.  Our final attempt to
...
rule present in most unfocused sequent calculus systems, can help us
write dowm compact proofs. If $F(n) = p_1^+ \otimes \ldots \otimes
