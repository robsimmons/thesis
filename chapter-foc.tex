...
(Section~\ref{sec:introlinlog}) as a logic with the ability to express
aspects of state and state transition in a natural way. In
Section~\ref{sec:linlogicalframeworks}, we discuss why the normal
presentation of linear logic is insufficient as a {\it logical
  framework} -- proofs in linear logic suffice to establish the
existance of a series of state transitions but do not adequately
...
transitions. There are still two deficiencies, however. The first
defeciency is that the natural interpretation of positive atomic
propositions does not allow us to express certain desirable state
transitions. We propose a modification to the focused sequent calculus
that addresses this first problem in Section~\ref{sec:linhack}; we
also observe that the resulting system is less natural and discuss the
implications of that observation. The second deficiency is that
focusing gives us a highly sequential transition system, whereas our
goal is to give an interpretation of concurrent transition systems. In
Section~\ref{sec:linconcurrenteq}, we describe {\it concurrent
  equality}, an equivalence relation on focused derivations introduced
by Watkins et.~al to solve this problem, and also consider the
limitiations of concurrent equality in a fully focused setting.

In Chapter 3, we will define a much richer ordered lax logic and prove
a focusing result that incorporates the atom and bang optimizations
from Section~\ref{sec:linhack}. In Chapter 4 we will carve out a
...
within the comparatively simple setting of intuitionstic linear logic.
...
heavily-shifted propositions like
...
in Section~\ref{sec:lincorrectness}, so we will focus a polarized
linear logic with shifts.
...
$A^\oplus$.\footnote{The two exceptions are Zeilberger completeness
  ...
  proof in intuitionistic persistent logic
  \cite{simmons11structural}.}  The weaker formulation is sufficient
for our current purposes, so we will discuss, in
Section~\ref{sec:lincorrectness}, the weaker property of
polarization-strategy-based correctness, not erasure-based
correctness.\robnote{Revisit this decision; It really seems like it
  might be easier to just describe erasure.}

\subsection{Focused sequent calculus}

Usually, focused logics are described as having multiple
sequent forms. For intuitionstic logics, there are at least three
sequent forms:
\begin{itemize}
...
negative propositions $\langle A^- \rangle$. 
...
it is an instacne of an admissible identity theorem:
$\seq{\Gamma}{A}{A}$ for all $A$. If we substitute in a concrete
...
become admissible instances of the general identity theorem.
...
counterintuitive and relies on a very higher-order use of LF's unform
...
\begin{proof}
Mutual induction over the strucure of the proposition $A^+$ or $A^-$,
...
expansion. The usual identity principles are simple
...
give a tidier proof of the theorem.\footnote{The main ``untidy''
  ...
  and 4. The repetition of almost all cases between parts 3 and 5 will
...
propositions. However, our current objective is not simply the study
of focused logic; we want to use the structure of proofs in focused
...
system more expressiveness at the cost of the elegant
...
immediately blurs, as in the this proof fragment:\footnote{We will use
...
\label{sec:linconcurrenteq}
 ...
 The third, concurrent equality, is a
