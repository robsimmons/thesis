...
introduction of {\it permeable atoms}, will be generalized and
incorporated into the focused presentation of ordered linear lax logic
...
This unfocused cut rule is independently interesting, and in any case
...
propositions. In this section, I will discuss four changes:
\begin{itemize}
\item Andreoli's original system, which I name the {\it atom optimization},
that complicates the interpretation of atomic propositions as stand-ins
for arbitrary propositions;
\item A further change to the atom optimization, the {\it bang optimization},
which complicates the relationship between the ;
\item The {\it adjoint logic} of Benton and Wadler
\cite{benton96linear}, which introduces
a new syntactic class of persistent propositions; and
\item The {\it atom refinement}, a notion (which dates
at least back to Girard's LU \cite{girard93unity}),
that some propositions can be treated as {\it permeable} between
the persistent and linear contexts and that permeable atoms can be introduced
to stand for 
\end{itemize}

 difference between
the system presented in Section~\ref{sec:foclinlog}, Andreoli's 
modification (which I call the {\it atom optimization}), and another
system, based on adjoint logic, that I have used in previous work
with Pfenning.

The goal of both of these systems (Andreoli's atom optimization and
focused adjoint logic) is essentially the same: to allow
for synthetic inference rules of the form
\[
\infer
...
My justification for presenting
Chaudhuri's system as the canonical focusing system for linear logic
in Section~\ref{sec:foclinlog} is because it most easily facilitates 
reasoning about the focused 
sequent calculus {\it as a logic}. Internal soundness and 
completeness properties are established by the cut admissibility
and identity expansion theorems, and these theorems are conceptually
prior to the soundness and completeness of the focused system relative
to the unfocused system. The two modifications we discuss in this section
complicate the treatment of focused logics as independently justifiable
sequent calculi for linear logic. Andreoli's atom optimization 
complicates cut admissibility for the focused, 
and adjoint logic makes it is difficult or impossible to
define a well-behaved erasure function so that that 
polarization strategies -- partial inverses of erasure -- exist.
...
immediately blurs, as in this (intuitionistic) 
proof fragment:\footnote{We will use
...
Introducing the atom optimization into our polarized system through a
new connective also allows us to isolate the effects that this new
connective has on cut admissibility and identity expansion; we will consider
each in turn.
...
Even though the identity expansion theorem is unproblematic, the
suspended persistent propositions let us consider the first
objection to the atom optimization. 
...
proposition; the most obvious candidate is
${!}{\uparrow}A^+$. That substitution requires us to change the
structure of proofs in a significant way, however, as shown in 
...
of this pattern arises when we try to establish the
...
$\langle p^+ \rangle$ does not appear in the linear context. 
We need to get enough $\langle p^+ \rangle$ propositions
in the linear context replace all the instances of
${\it id}_2^+$ that deal with the suspended $p^+$ in the persistent
context with instances of ${\it id}_1^+$ that use freshly-introduced
$\langle p^+ \rangle$ propositions. This can be achieved with 
repeated application of part 3 of Theorem~\ref{thm:lincut}, as shown
in the bottom half of Figure~\ref{fig:bad-cut}.

The minimal extension to Theorem~\ref{thm:lincut} that justifies 
the atom optimization appears to be the following, where 
$\langle p^+ \rangle^n$ denotes $n$ copies of the suspended positive
...
\paragraph{Soundness and completeness of focusing}
...
Completeness requires that we prove two new unfocused admissibility rules
dealing with ${\pbang}$,
...
admissibility is significantly simpler. In the principal cut for
...
with the bang optimization. It is not, however,
...
Our focused logic with the bang optimization has some resemblance to
tensor logic \cite{mellies10resource}, as well the polarized
logic that Girard presented in a note, ``On the sex of angels,''
which first introduced the ${\uparrow}A^+$ and ${\downarrow}A^-$
notation to the discussion of polarity \cite{girard91sex}. Both of
these presentations incorporate a general focus-preserving
${\pbang}A^+$ connective -- a positive formula with a positive
subformula -- in lieu of the focus-interrupting ${!}A^-$
connective.
Both presentations also have the prominent caveat that
${!}$ in the unfocused logic necessarily corresponds to
${\pbang}{\downarrow}$ in the focused logic: it is {\it not} possible
to derive ${\pbang}(A \otimes B) \vdash {\pbang}(B \otimes A)$ in
these systems, and no apology is made for this fact, because
${\pbang}{\downarrow}{\uparrow}(A \otimes B) \vdash
...
proofs.\footnote{Both the note of Girard and the paper of Melli{\`e}s
  ...
  proposition $A$ that is classically but not intuitionistic true,
  we know that the double negation $\neg\neg A$ will be true in
  ...
  viewpoints \cite{zeilberger08unity}.} This interpretation of 
shifts therefore threatens our ability to understand linear logic 
connectives as resources.
...
the linear context and always appear as ${\pbang}p^+_p$. For {\it
  these} sequents, we can use the obvious erasure operation and obtain
a focalization result; this notion of separation was the basis of our
completeness result in \cite{pfenning09substructural}.  However,
separation is a meta-logical property, something that we observe about
a fragment of the logic and not than an inherent property of the logic
itself. There are many propositions $A^+$ that we cannot prove in
focused linear logic with the bang optimization even though
$(A^+)^\circ$ is provable in linear logic, and that makes the
bang optimization unsatisfactory. 

The remaining two approaches, adjoint logic and the atom refinement
approach, can potentially be seen as two ways of trying to turn the
separation criteria into a logical property instead of a meta-logical
property.
...
atom and bang optimizations, may be faithfully encoded.
...
The separation criteria discussed above can then be understood by
reintroducing atomic propositions $x$ into the linear logic
fragment of adjoint logic. 
Then, positive atomic propositions that are always associated with
...
In this way, adjoint logic gives one answer to why, in linear logic,
...
logic, adjoint logic, between the focused system and the structure of
linear logic that we actually hope to express.  Another drawback of
this approach is that, in order to give a fully focused presentation
...
The admissible identity expansion rule can be very helpful as a way
...
$\Delta$ has lots of complicated inversion to do, we can suspend
...
is an instance of this pattern, for example:
\[
\infer-[\eta^+]
{\mildseq{\Gamma}
  {{\downarrow}A 
...
{\infer[{\it focus}_R]
 ...
 {\infer[{\downarrow}_R]
  ...
  {\infer[{\top}_R]
   ...
   {}}}}
\]
The use of identity expansion lets us delay the decomposition that
would otherwise be required by focusing in order to do more useful
work. Another pattern that we might want to capture with something
like identity expansion is the situation where we are trying to prove
the sequent $\mildseq{\Gamma}{\Delta}{1}$ and we know, by the syntactic
structure of $\Delta$, that inversion will empty the context; one 
instance of this pattern is the sequent 
$\mildseq{\Gamma}
  {{!}A 
   \otimes {!}B 
   \otimes {!}C 
...
In general, it is possible to describe the syntactic refinement of the 
positive MELL propositions with this property:
\[
...
We can then augment our original focused linear logic from
Figure~\ref{fig:kaustuv-focused} with one extra rule, ${\it id}^+_p$
which exactly matches our earlier rule ${\it id}_2^+$. This rule gives
us a new focal substitution principle ${\it subst}_p^+$, which was
true but unnecessary in the previous systems with ${\it id}_2^+$ rule.
Using this focal substitution principle, we can prove a {\it
  persistent} identity expansion theorem on this syntactic refinement
...
\]
The proof of the persistent focal substitution theorem is once again
a straightforward induction over the derivation over the structure
of $\mildseq{\Gamma, \langle A^+ \rangle}{\underline{\Delta}}{\underline{U}}$,
and the proof of the persistent identity expansion, the cases of
which are given in Figure~\ref{fig:lineta-3}, is by induction on the
structure of our refined fragment of positive propositions.

\input{figs/fig-lineta-3}

So far, we have only added one rule to our logic, ${\it id}^+_p$, 
and it is not a rule
that influences the existing cut admissibility or identity expansion
principles in any way, even though it does allow the introduction of a
new identity expansion principle. We
can now use this persistent identity expansion property to
give a compressed proof of our example sequent:
\[
\infer-[\eta^+_p]
{\mildseq{\Gamma}
  {{!}A 
...
Our real motivation for introducing this identity expansion principle
was to introduce a corresponding class of atomic propositions $p^+_p$ that
can be interpreted as arbitrary members of our syntactic refinement in the
same way that regular positive atomic propositions can be interpreted
as arbitrary positive propositions. Persistent atomic propositions $p^+_p$
belong in the refinement $A^+_p$, which means that they belong in the
grammar of general positive atomic propositions $A^+$ as well; the rule
\[
\infer[\eta^+_p]
...
\paragraph{Identity expansion}
The new addition to the language of positive propositions does require us to 
...
{\infer-[{\it subst}^+_p]
 ...
 {\infer[{\it id}^+_p] 
  ...
  {\mildseq{\Gamma}{\Delta, \langle p^+_p \rangle}{U}}}}
\]
Other than this, identity expansion remains unchanged; the additional
case of persistent identity expansion that is introduced is exactly
handled by the new $\eta^+_p$ rule.

\paragraph{Cut admissibility}
The necessary modifications to the 
cut admissibility theorem are essentially the same as the ones needed
to prove cut admissibility for the bang optimization; the commutative
cuts are unproblematic, and the principal cut reduces to an
instance of contraction. 

\paragraph{Soundness and completeness of focusing} There are two ways
we can understand the soundness and completeness of focusing for linear
logic extended with permeable atomic propositions: first, we
can add a notion of permeable atomic propositions to our core 
linear logic from Figure~\ref{fig:linear}, in which case soundness and
completeness are straightforward. Alternatively, we can use our intuition
that a permeable proposition $A^+_p$ should be interprovable with 
${!}{\downarrow}A^+_p$ and let $(p^+_p)^\circ = {!}p^+_p$; 
we must also let $\langle p^+_p \rangle$ in the persistent context
erase to $p^+_p$.
