...
non-canonical LF term and the Canonical LF term. The statment of the
...
\cite{simmons11structural} on an intuitonistic proposition,
%
   $a^+ \supset 
...
in other words, is not a partial inverse of erasue in the structural
...
equal if and only if the original proofs are convertable by local
permutations of rules, the same canonicity condition that
...
then focalizes them -- which is what the Twelf implementation of LF
