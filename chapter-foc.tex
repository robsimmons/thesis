...
Addressing this nondetermism within aninversion phase echos the
discussion of LF from the beginning of the chapter. 

We can say, as suggested in that introduction, decleare that all
...
Then, remenicient of the move from LF to Canonical LF, the logic
itself can be restricted so that only the canonical representatives
are admitted. The most convienent way of forcing a left-most,
...
fair amount of burecracy compared to the original
...
observation behind synthetic inference rules is that the stable
...
strategy used in the completeness of linear logic, which only produced
stable sequents. The second observation is that, once we know 
whether the last rule in the proof 
is ${\it focus}_L$ on some proposition $A^-$ in $\Delta$,
${\it copy}$ on some proposition $A^-$ from $\Gamma$, or 
${\it focus}_R$ on the conclusion $A^+$ (it must be one of these
three), then the structure of the proof is completely determined
up to the next occurance of a stable sequent. 
...
correspondance with proofs using synthetic inference rules. If we look
at our running example as a derivation using inference rules, we see
that the system takes four steps. The last two steps, furthermore,
correspond precisely to the intermediate steps in our informal
...
zero, as would be the case for right focus on $\zero$).
