...
\trstep{y_1,y_2}{{\sf string/right}\,y}; T')$ does not syntatically
...
implicitly rely on concurrent equality in theis way, the resulting
...
\begin{proof}
The proof is by case analysis on the structure of $S$; it relies on four
lemmas about the structure of $T_1$. 
...
\item $T = (\trstep{x_1,x_2}{{\sf stack/left}\,x}; T'')$. By the
  induction hppothesis (where the new frame incorporates
...
\noindent
A corrolary of this lemma is that if 
...
analysis on the last steps in $T$; we will give the first proof, as
the other two are similar. Up to concurrent equality, there are five
possibilities:
\begin{itemize}
\item $T = T''; S$, where $x_1$ and $x_2$ are not bound by $S$. By the 
induction hypothesis, $T'' = (T'''; \trstep{x_1,x_2}{{\sf stack/left}\,x'})$.
Let $T' = (T''; S)$. 
\end{itemize}
Note
