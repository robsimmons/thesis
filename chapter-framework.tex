...
Licata's Canonical LF presentation insetad of presenting Spine Form
...
considerations. First and foremost, this choice was sufficent for our
purposes in this thesis. In fact, for the purposes of this thesis, we
...
langauges. Canonical LF and Spine Form LF are, at this point,
...
are two examples; most concievable simply-typed term languages would
...
presentations of logic, but there is a wrinkle in this account if we
try to apply this story to a dependently type theory like CLF that
does not distinguise object terms and proof terms. CLF terms have a
notion of {\it concurrent equivalence} which is coarser the
equivalence relation described by focusing, and we want \sls~proof
terms to have a similar notion of equivalence. In \sls, we can
characterize concurrent equivalence in Section~\ref{XXX}, after
completing our discussion of the framework and describing the proof
term language.  In CLF, because the framework's definition discusses
term equality, and equality of terms must take concurrent equivalence
into account, so concurrent equivalence of (proof) terms is
conceptually prior to the logic itself. We conjecture that this
complication is no great obstacle, but this thesis avoids the issue.

On a practical level, 

The restriction to a dependent type theory therefore has low overhead,
and there is additionally reason to suspect that . A first-order
version of LF would be able to {\it represent} many of the systems we
are interested in, and in fact such a system is sufficient for
settings such as Harper's comprehensive survey of programming language
design \cite{harper12practical}.\footnote{Harper's metatheory also
  extends LF by drawing a distinction between standard variables and
  nominal parameters, but this is an orthogonal point.} 


In addition to . 

The type system for \sls~requires that we be able to decide equality
of terms in our domain of first-order quantification; this is simple,
because equality of 

presentation of
\sls~given in this chapter requires

as a term language originated with the idea

\subsection{Core syntax}

The substructural portion of \sls~imposes on our definition of LF at
two points: first, signatures $\Sigma$ contain not only 


declare not only LF term
constants $\lf{\sf c}$ and LF type family constants ${\sf a}$ but
predicates (which have the same metavariable -- ${\sf a}$ -- as type
constants) and rules ${\sf r}$, which are classified by negative types
$A^-$.  The kind ${\sf type}$ classifies types, so the declaration
${\sf a} : \lfpi{x_1}{\tau_1}{\ldots \lfpi{x_n}{\tau_n}{\sf type}}$
