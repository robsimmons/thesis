...
prove.\footnote{The term {\it top-down logic programming} is also
  used, and refers to the fact that, in Prolog, the rule
  ${\downarrow}p_n^- \lefti \ldots \lefti {\downarrow}p_1^- \lefti
  p^-$ would be written with $p^-$ on the first line, $p_1^-$ on the
  second, etc. This is counterintuitive from a proof-construction
  perspective, as we are building partial proofs from the bottom
  up. Therefore, we will avoid this terminology, as well as the
  terminology {\it bottom-up logic programming}, which is a synonym
  for {\it forward chaining}.}  The backward-chaining interpretation
of intuitionistic logics dates back to the work by Miller et al.~on
uniform proofs \cite{miller91uniform}.  An even older concept, Clark's
{\it negation-as-failure} \cite{clark87negation}, stipulates that if a
...
to conclude that no proof exists. This interpretation requires that
backward chaining include {\it backtracking}: if we we try to prove
...
declarations. Non-backtracking goal-oriented deductive computation is
called {\it flat resolution} \cite{aitkaci99warrens}.

Another feature of backward chaining is that is
allows for unspecified variables. Consider the following \sls~signature:
\begin{align*}
 \Sigma_{\it Add} = \cdot, 
~&{\sf nat} : {\sf type}, 
~~\lf{\sf z} : {\sf nat}, 
...
mode). A predicate can sometimes be given more than one mode: 
...
Trace computation is usually implemented by allowing an initial
process state $(\Psi;\Delta)$ to evolve freely by applying synthetic
transitions.  In persistent logics, this {\it forward chaining}
interpretation of has provided the basis for using logic programs to
describe deductive databases.  Forward-chaining trace computation in
substructural logics is {\it committed choice} and not backtracking:
once we observe that $(\Psi;\Delta) \leadsto (\Psi'; \Delta')$, we try
to evolve the state $(\Psi'; \Delta')$, and we do not consider what
other process states could be obtained by evolving $(\Psi;\Delta)$.
Forward chaining does not consider unspecified
...
logics. In previous work, we designed a forward chaining interpreter
...
Another paradigm for trace computation is model checking. The model
checking problem takes an intial process state $(\Psi;\Delta)$ or a
set of initial states and enumerates or otherwise characterize the
entire set of states $(\Psi'; \Delta')$ such that $(\Psi; \Delta)
\leadsto^* (\Psi'; \Delta')$. In the special case where specifications
are entirely persistent, the forward chaning and model checking
are essentially the same; we will call this {\it saturating} logic 
programming and discuss it further in Chatper~8. 

The rough classification we have given of deductive computation and
trace computation is non-standard. One reason for this is that most of
the definitions in the literature consider only persistent logics, and
substructural logics tend to reveal distinctions that are not present
in persistent logic. Furthermore, we have left out several important
classes of deductive computation, like tabled logic programming and
breadth-first goal-oriented search. (The goal-oriented search we have
described is depth-first.) Nevertheless, the rough sketch we have
presented is presented graphically in
Figure~\ref{fig:computation-taxonomy}.

\begin{figure}
\[\mbox{\Huge TODO}\]
\caption{A rough taxonomy of deductive and trace computation}
