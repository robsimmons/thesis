...
of quantification was \sls~terms, these three considerations 

The thesis of this
and the previous two chapters, that we can productively derive logical
frameworks from focused presentations of logic, is a statement about
the second step in Section~\ref{sec:slsframework}. 

XXX


It is the thesis of
this and the previous two chapters that we can productively derive
logical frameworks from focused presentations of logic, but doing so
implicitly requires that we understand equality on the term language.
Concurrent equality is {\it not} derived from focused \ollll, 



, but concurrent
equality complicates this story.

Spine Form LF and Canonical LF
are two examples, but most conceivable term languages without
concurrent equality would also suffice. 

it is not derived from focused \ollll, and applying concurrent
equality to \sls~terms breaks the bijective correspondence between
\sls~and the corresponding syntactic fragment of \ollll. The
presentation in this chapter treats the definiton of \sls~as
conceptually prior to the presentation of concurrent equality. This
would no longer be possible if \sls~terms were also our term language:
the logic's definition relies on knowing equality of proof terms,
 
 but this simple is complicated by the interaction
of full dependency and 

when we apply it
to a dependent type theory like CLF that does not distinguish object
terms and proof terms. Because equality of types depends on equality
of the term language, CLF 

in CLF depends on



CLF terms have a notion of {\it concurrent
...
In Section~\ref{sec:linlogtrans}, we presented 
