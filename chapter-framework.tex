...
Figure~\ref{fig:sls-propform}.  As discussed in the introduciton to
...
Much of the complexity of Figure~\ref{fig:sls-propform}, in particular
the existance of an additional context $\mathcal S$ that is treated
like a multiset, is thanks to the inclusion of equality \sls.  The
restrictions ensure that, whenever we decompose a positive proposition
$\lf{s} \doteq \lf{t}$, we have that $\lf{s} = \lf{a}$ for some
variable $\lf{a}$ in the context. When this is the case, $\lf{[t/a]}$,
is always a most general unifier of $\lf{s} = \lf{a}$ and $\lf{t}$,
which in turn means that the left rule for equality in
\ollll
\[
\infer[{\doteq}_L]
...
\]
Usually, when we require the ``existance of most general unifiers,''
...
equality is used as a {\it notational definition} and when it is used
as
