...
\ollll~proof terms.  Additionally, we have a single $\lambda P.N$
constructor and a single $V;\Sp$ spine rather than a term $\tlamr{N}$
and spine $\tappr{V}{\Sp}$ associated with propositions $A^+ \righti
B^-$ and another term $\tlaml{N}$ and spine $\tappl{V}{\Sp}$
associated with propositions $A^+ \lefti B^-$.  As with patterns, it is
possible to think of these terms as just having extra annotations
($\lambda^>$ or $\lambda^<$) that we have omitted.
...
\end{align*}
A step $S$ corresponds precisely to the notion of a {\it synthetic inference
rule} discussed in Section~\ref{sec:linsynthetic}. It captures a 
focusing phase (contained in the spine $\Sp$ 
\[
\infer
{\tstep{P}{x}{\Sp} :: 
...
we can also call steps {\it synthetic transitions}. Traces $T$ are monoids
over steps -- $T$ is an empty trace, $S$ is a trace consisting of a 
single step, and $T_1; T_2$ is the composition of traces. 

Concurrent traces can be included into deductive terms by the 
term $N ::= \ldots \mid \tlet{T}{V}$ associated with the right rule
for equality. 
\[
\infer[{\ocircle}_R]
