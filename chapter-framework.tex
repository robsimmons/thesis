...
$\Sigma$ implicit when it is unambiguous or uninportent. We will
...
Similar frame properties hold for \sls~traces. The direct analouge of
...
is a generalization of strenghtening to \sls~traces.
...
that $\ctxinterp{k \rhd s} \leadsto_{\Sigma_{\it PDA}} \ctxinterp{k'
  ...
  s'}$. The second step is cruicial: without it, we might transition
in \sls~from the encoding of some $\obj{k \rhd s}$ to a state
$\Delta'$ that is not in the image of encoding.

In this section, we re-factor this methadology to use {\it generative
  signatures}. Generative signatures are a rich generalization of {\it
  regular worlds} that can be used to describe invariants of
\sls~traces, a topic we will explore further in Chatper~9.  The first
step in our revised methadology is to describe a generative signature
$\Sigma_{\it Gen}$ that precisely captures the set of process states
that encode machine states (Theorem~\ref{thm:pda-encoding} below).  The
second step is showing that the generative signature $\Sigma_{\it
  Gen}$ describes an invariant of the signature $\Sigma_{\it PDA}$
(Theorem~\ref{thm:pda-preservation}).
because this invariant captures the set process states that encode
PDA states, we therefore know that a process state that encodes
a PDA state can only step to another process state that encodes 
a PDA state. The third step, showing that 
$\ctxinterp{k \rhd s} \leadsto_{\Sigma_{\it PDA}} \ctxinterp{k'
  \rhd s'}$ if and only if $\obj{k \rhd s} \mapsto \obj{k' \rhd
  s'}$, is the same as in other developments.
1
...
based on context-free grammars. Convienently, these contexts can be
...
\begin{theorem}[Encoding]\label{thm:pda-encoding}
  ...
  \Delta'$ and $\Delta'$ matches $\restrictto{\Delta}{\Sigma_{\it
      PDA}}$.
\end{theorem}
...
\smallskip
If $\obj{k \rhd s} \neq \obj{k' \rhd s'}$ then 
$\ctxinterp{k \rhd s} \neq \ctxinterp{k' \rhd s'}$, 
and for all $\obj{k}$ and $\obj{s}$ the 
process state $\ctxinterp{k \rhd s}$ matches 
$\restrictto{\ctxinterp{k \rhd s}}{\Sigma_{\it PDA}}$ -- the function we have
defined only includes terminals. Therefore, to show that the encoding function
is injective we must show that, for any $\obj{k \rhd s}$,
there exists a trace $T :: (x{:}\istrue{\susp{\sf gen}})
\leadsto^*_{\it Gen} \ctxinterp{k \rhd s}$. To show that the encoding
function is surjective, we must show that if 
$T :: (x{:}\istrue{\susp{\sf gen}}) \leadsto^*_{\Sigma_{\it Gen}}
  \Delta'$ and $\Delta'$ matches $\restrictto{\Delta}{\Sigma_{\it
      PDA}}$ then $\Delta = \ctxinterp{k \rhd s}$ for some $\obj{k}$
and $\obj{s}$. (An injective and surjective function is bijective.)
...
\end{lemma}
\noindent
By straightforward construciton using the first two lemmas
...
$T :: (\ctxinterp{k}, 
       ...
       \ctxinterp{s}) 
  \leadsto^*_{\Sigma_{\it Gen}} \Delta'$ and $\Delta'$ matches
$\restrictto{\Delta}{\Sigma_{\it
      PDA}}$ then $\Delta = \ctxinterp{k' \rhd s'}$ for some $\obj{k'}$
and $\obj{s'}$.
\end{lemma}
\noindent
By induction on the trace $T$, taking concurrent equality
into account. 

If $T = (\trstep{()}{{\sf stack/done}\,x}; \trstep{()}{{\sf state/done}\,y})$
(or the concurrently equal trace where those two steps are given
in the other order) then we are in a base case, and we succeed letting
$\obj{k'} = \obj{k}$ and $\obj{s'} = \obj{s}$. Otherwise, $T$ is concurrently
equal to one of the following:
\smallskip
\begin{itemize}
\item $T = \trstep{x_1,x_2}{{\sf stack/left}\,x}; T'$,
\item $T = \trstep{y_1,y_2}{{\sf string/left}\,y}; T'$, or
\item $T = \trstep{y_1,y_2}{{\sf string/right}\,y}; T'$.
\end{itemize}
\smallskip
In all three cases the result follows directly from the induction
hypothesis on the smaller trace $T'$. 
...
be the case that $T = \trstep{}{}; T'$. We can apply the previous
...
signature only take encoded PDA states to encoded PDA states.
 
\bigskip
\begin{theorem}[Preservation]\label{thm:pda-preservation}
If $T_1 :: (x{:}\istrue{\susp{\sf gen}}) \leadsto^*_{\Sigma_{\it Gen}} \Delta'_1$,
the process state
$\Delta'_1$ matches $\restrictto{\Delta_1}{\Sigma_{\it PDA}}$, and 
$\Delta_1 \leadsto_{\Sigma_{\it PDA}} \Delta_2$, then 
\end{theorem}
\bigskip
...
for the SLS language; code that is checked by this prototype
implementation will appear repeatedly in the rest of this thesis,
always in a \verb|fixed-width font|.
...
a few ways. Following CLF and the Celf implementation, we write
${\ocircle}A$ in Celf as \verb|{A}|. The mobile modality ${\gnab}A$
doesn't have an ASCII representation, so write \verb|$A| when $A$ is
...
are part of the \sls~fragment anyway. The translation between the
mathematical notation we use for \sls~propositions and the ASCII
representation used by the implementation is outlined in
Figure~\ref{fig:translate-types}.
...
expected. This allows us to write either \verb|a * b * c| or
...
(respectively). Without this information, it becomes difficult to tell
how order, linearity, and persistence interact in specifications that 
...
Leave the question of quiescence versus eagerly-trying-to-right-focus
