...
Chapter~3; in Section~\ref{sec:sls-termlanguage} we give a careful
presentation of the term language for \sls, Spine Form LF. In
...
encoding to \sls. In Section~\ref{sec:prototype} we cover the
\sls~prototype implementation, and in
Section~\ref{sec:framework-logicprog} we review some intuitions about
logic programming in the framework. Finally, in
...
While we will always include the annotations for signature $\Sigma$
...
The {\it frame rule} is a concept from Reynold's separation logic.  It
...
extra state framed on to the outside. This is essentially a
...
\begin{theorem}[Frame strengthening]\label{thm:framestrong}~\\
  If $T :: (\Psi; \tackon{\Theta}{x{:}\islvl{T}}) \leadsto^*_{\Sigma,
    \subord} (\Psi'; \tackon{\Theta}{x{:}\islvl{T}})$ and $x$ is not
...
\sls~proofs when we factor the latter modulo concurrent equality
...
terms of strings $\obj{s}$ defined as $\obj{s} ::= \obj{\cdot} \mid
\obj{{\sf <}s} \mid \obj{{\sf >}s}$ and stacks $\obj{k}$ defined as
$\obj{k} ::= \obj{\cdot} \mid \obj{k{\sf <}}$ with two rules: $\obj{(k
  \rhd ({\sf <}s))} \mapsto \obj{((k{\sf <}) \rhd s)}$ and
...
y_4{:}\istrue{\susp{\sf <}}
\]
The second step is to prove a preservation-like adequacy theorem: if
state $\ctxinterp{\obj{k \rhd s}} \leadsto_{\Sigma_{\it PDA}}
...
that $\ctxinterp{k \rhd s} \leadsto_{\Sigma_{\it PDA}} \ctxinterp{k'
  ...
  s'}$. The second step is crucial: without it, we might transition
in \sls~from the encoding of some $\obj{k \rhd s}$ to a state
$\Delta'$ that is not in the image of encoding. 

In the case of our deterministic PDA, this concern is less readily
apparent, because we could avoid the second proof by instead proving
that if $\ctxinterp{k \rhd s} \leadsto_{\Sigma_{\it PDA}} \Delta$ and
$\ctxinterp{k \rhd s} \leadsto_{\Sigma_{\it PDA}} \Delta'$ then
$\Delta = \Delta'$. In this section, we will use this
preservation-like property to motivate a topic we will discuss further
in Part~3 of this thesis. This presentation re-factors the methodology
given here by using {\it generative signatures}. Generative signatures
are a rich generalization of {\it regular worlds} that can be used to
describe invariants of \sls~traces.  The first step in our revised
methodology is to describe a generative signature $\Sigma_{\it Gen}$
that precisely captures the set of process states that encode machine
states (Theorem~\ref{thm:pda-encoding} below).  The second step is
showing that the generative signature $\Sigma_{\it Gen}$ describes an
invariant of the signature $\Sigma_{\it PDA}$
(Theorem~\ref{thm:pda-preservation}).  because this invariant captures
the set process states that encode PDA states, we therefore know that
a process state that encodes a PDA state can only step to another
process state that encodes a PDA state. The third step, showing that
$\ctxinterp{k \rhd s} \leadsto_{\Sigma_{\it PDA}} \ctxinterp{k' \rhd
  s'}$ if and only if $\obj{k \rhd s} \mapsto \obj{k' \rhd s'}$, is
the same as in other developments.
...
signatures respects a particular world structure
...
\cite{polakow01ordered}. Our PDA sates, on the other hand, do not have
a regular structure, because the process state is organized into three
zones:
\[
[~\mbox{the stack}~]
~
...
$\Sigma_{\it Gen}$ in Figure~\ref{fig:pda-gen} treats all the elements
of $\Sigma_{\it PDA}$ -- the atomic propositions ${\sf <}$, ${\sf >}$
and ${\sf hd}$ -- as {\it terminals}, and introduces three {\it
...
that holds whenever the partial function is defined.
...
function; it remains to be shown that the function is surjective. 
...
\item $T = (\trstep{y_1,y_2}{{\sf string/right}\,y}; T')$ -- apply
  ...
  \obj{{\sf >}s}$).
\end{itemize}
\smallskip In all three cases the result follows directly from the
induction hypothesis on the smaller trace $T'$.  The proof above takes
a number of facts about concurrent equality for granted. For example,
the trace $T = (\trstep{()}{{\sf stack/done}\,x};
\trstep{y_1,y_2}{{\sf string/right}\,y}; T')$ does not syntactically
match any of the traces above, but it matches the last case because $T
= (\trstep{y_1,y_2}{{\sf string/right}\,y}; \trstep{()}{{\sf
    stack/done}\,x}; T')$ modulo concurrent equality. If we didn't
implicitly rely on concurrent equality in this way, the resulting
proof would have twice as many cases.  We will take these finite uses
of concurrent equality for granted when we specify that a proof
proceeds by case analysis on the first steps of $T$ (or, conversely,
by case analysis on the last steps of $T$).
...
this theorem in our two-dimensional notation as follows, where the
$\Delta \Leftrightarrow \obj{k \rhd s}$ indicates the presence of the
bijection:
\begin{center}
\begin{tikzpicture} 
...
Theorem~\ref{thm:pda-encoding} took advantage of the monoidal
...
and $\restrictsig{\Delta_1}{\Sigma_{\it PDA}}$.
\end{theorem}
\bigskip
...
transitions associated with a given signature. Then, in each of those
cases, we use the type of the synthetic transition to perform
inversion on the structure of the given generative trace until we
obtain a trace that we can use to construct a generative
...
prototype implementation will appear repeatedly in the rest of this
...
The mobile modality ${\gnab}A$ doesn't have an ASCII
...
use these option annotations in a consistent way.  We will omit the
...
propositions. This making the mixture of different exponentials
...
The logic programming interpretation of CLF was explored by the
...
programming intuitions motivated the design of \sls, in particular the
separation of traces from the deductive fragment, as we will discuss
in Section~\ref{sec:framework-logicprog-trace}.
...
of sequents of the form $\foc{\Psi}{\Delta}{\istrue{\susp{p^-}}}$.
The most common form of deductive computation is {\it goal-directed}
computation.  In goal-directed computation, we will only try to prove
the sequent $\foc{\Psi}{\Delta}{\istrue{\susp{p^-}}}$ by focusing on a
proposition like ${\downarrow}p_n^- \lefti \ldots \lefti
{\downarrow}p_1^- \lefti p^-$ which has a head $p^-$ that matches the
succedent. This replaces the goal
$\foc{\Psi}{\Delta}{\istrue{\susp{p^-}}}$ with $n$ goals
$\foc{\Psi}{\Delta_1}{\istrue{\susp{p_1^-}}}$ \ldots
$\foc{\Psi}{\Delta_n}{\istrue{\susp{p_n^-}}}$, where $\Delta$ matches
$\Delta_1,\ldots,\Delta_n$.

Goal-directed deductive computation is also called {\it backward
  chaining}, because we're working backwards from the goal we want to
prove.\footnote{The term {\it top-down logic programming} is also
  used, and refers to the fact that the rule ${\downarrow}p_n^- \lefti
  \ldots \lefti {\downarrow}p_1^- \lefti p^-$ would be written with
  $p^-$ on the first line, $p_1^-$ on the second, etc. This is
  counterintuitive from a proof-construction perspective, as we are
  building partial proofs from the bottom up. Therefore, we will avoid
  this terminology, as well as the terminology {\it bottom-up logic
    programming}, which is a synonym for {\it forward chaining}.}  The
backward-chaining interpretation of intuitionistic logics dates back to
the work by Miller et al.~on uniform proofs \cite{miller91uniform}.
An even older concept, Clark's {\it negation-as-failure}
\cite{clark87negation}, stipulates that if the logic programming
interpreter reports that it has failed to find a proof of a
goal-directed sequent, we should be able to conclude that no proof
exists. This interpretation requires that proof search be {\it
  backtracking}: if we we try to prove
$\foc{\Psi}{\Delta}{\istrue{\susp{p^-}}}$ by focusing on a particular
proposition and one of the resulting subgoals fails, we have to
consider any other propositions that could have been used to prove the
sequent before giving up. Backtracking can be extremely powerful in
certain cases and incredibly expensive in others, and so most logic
programming languages have an escape hatch that modifies or limits
backtracking at the user's discretion, such as the Prolog cut (no
relation to the admissible rule ${\it cut}$) or Twelf's deterministic
declarations. Non-backtracking goal-oriented deductive computation
is called {\it flat resolution} \cite{aitkaci99warrens}.

Another feature of goal-oriented deductive computation is that is
allows for unspecified variables. Consider the following \sls~signatures:
\begin{align*}
 \Sigma_{\it Add} = \cdot, 
~&{\sf nat} : {\sf type}, 
~~\lf{\sf z} : {\sf nat}, 
...
succeed, reporting that $X = \lf{\sf (s\,(s\,z))}$.

We say that ${\sf plus}$ is a {\it well-moded} predicate with {\it
  mode} $({\sf plus}\,{+}\,{+}\,{-})$, because whenever we perform
deductive computation to derive $({\sf plus}\,\lf{n}\,\lf{m}\,\lf{p})$
where $\lf{n}$ and $\lf{m}$ are fully specified, any unspecified
variables in $\lf{p}$ will become fully specified in the course of
deductive computation. Well-moded predicates can be treated as partial
functions from their inputs (the indices marked ``${+}$'' in the
mode) to their outputs (the indices marked ``${-}$'' in the
mode). A predicate can, in general, be given more than one mode: 
$({\sf plus}\,{+}\,{-}\,{+})$ is a valid mode for ${\sf plus}$, but 
$({\sf plus}\,{+}\,{-}\,{-})$ is not. 

The implementation of goal-oriented deductive computation in various
substructural logics has been explored by Hodas \cite{hodas94logic},
Polakow \cite{polakow00linear,polakow01ordered}, Armel\'in and Pym
...
this potentially exponential backtracking, but specifying resource
management, especially for richer substructural logics, can be complex
and subtle \cite{cervesato00efficient}.
...
complete derivation from the top down, as it were. The inverse method
is generally associated with theorem proving and not logic
...
logic can simulate both backward-chaining deductive computation and
forward-chaining trace computation (considered below) for persistent
Horn-clause logic programs \cite{chaudhuri10logical}.
...
\Delta)_{\lf{\sigma}} \leadsto^* (\Psi'; \Delta')_{\lf{\sigma'}}$. 

Trace computation is usually understood in terms of specifying a
signature, an initial process state $(\Psi;\Delta)$, and then allowing
this initial state to evolve freely by applying rules from the
signature.  This {\it forward-chaining} interpretation of persistent
intuitionistic logic is the basis of Datalog. There is also significant
body of work in giving forward-chaining interpretations to linear
logic, including Forum \cite{miller96multiple} and Lollimon
\cite{lopez05monadic}. Other languages and programming paradigms, such
as multiset rewriting and Gamma, are defined independently of linear
logic but can be partially or completely described in terms of proof
search in linear logic \cite{paola96linear,cervesato09relating}.
Forward-chaining trace computation in substructural logics is {\it
  committed choice} and not backtracking: once we observe that
$(\Psi;\Delta) \leadsto (\Psi'; \Delta')$, we try to evolve the state
$(\Psi'; \Delta')$, and we do not consider what other process states
could be obtained by evolving $(\Psi;\Delta)$.

Forward-chaining trace computation does not consider unspecified
variables and is largely free of the resource management problems that
appear in backward-chaining deductive computation for substructural
logics. In previous work, we designed a forward-chaining interpreters
for linear logic that admits abstract reasoning about the asymptotic
complexity of logical specifications \cite{simmons08linear}, but this
is

We will be most interested in forward-chaining trace computation in
this thesis. Another paradigm is model checking, where we are given a
signature and an initial state $(\Psi;\Delta)$ and want to enumerate
or characterize the set of states $(\Psi'; \Delta')$ such that $(\Psi;
\Delta) \leadsto^* (\Psi'; \Delta')$. In the special case where
specifications are entirely persistent, this is {\it saturating} trace
computation.

\subsection{Integrating deductive and trace computation}

In logic programming interpreter used by Lollimon and Celf,
backtracking, goal-directed search is associated with the deductive
fragment, and trace computation is performed by committed-choice
forward chaining. The two sometimes have an uneasy relationship. To
see why, consider the following signature:
\begin{align*}
 \Sigma_{\it Demo} = \cdot, 
...
backtracking, goal-directed search is that an interpreter tasked with
finding a proof of ${\sf negD}$ should either find this proof or never
terminate. This is not what happens under the logic programming
interpretation used by Celf and Lollimon.  Deductive computation will
begin by trying to prove ${\sf negD}$; this can only be done with the
rule ${\sf bwdrule}$. Applying this rule generates the subgoal ${\sf
  posA} \lefti {\ocircle}{\sf posB}$ which Celf/Lollimon will attempt
to satisfy by constructing a trace $(x{:}\istrue{\susp{\sf posA}}) \leadsto
(y{:}\istrue{\susp{\sf posB}})$.
...
plausibly come up with either one. If it derives the second one, the
the subgoal fails, and a Celf/Lollimon-style interpreter will report
that it failed to prove ${\sf negD}$.

This example indicates that it is difficult to make backtracking
backward-chaining deductive computation reliant on committed-choice
forward-chaining concurrent computation, at least if we are interested
in preserving the former's usual negation-as-failure
interpretation. In the other direction, it is entirely natural to make
forward-chaining computation dependent upon deductive computation. The
fragment of CLF that encodes this kind of computation was called the
{\it semantic effects} fragment by DeYoung \cite{deyoung09reasoning}.

\section{Design decisions}
\label{sec:designdecisions}

Aside from ordered propositions, the main differences between the
framework \sls~presented in this chapter the existing logical
framework CLF are the presence of positive atomic propositions, the
introduction of traces as an explicit notation for partial proofs, the
restriction of the term language to LF, and the presence of
unification. In this section, we will discuss design choices
that were made in terms of each of these features, their effects, and
what choices could have been made differently. 

\subsection{Pseudo-positive atoms}
\label{sec:pseudopositive}

Unlike \sls, existing frameworks like CLF do not include positive
...
all linear atomic propositions, then the presence of a rule ${\sf foo}
\fuse {\sf bar} \lefti {\ocircle}{\sf baz}$ in the signature is associated
...
more generally than \sls's regular positive atomic propositions. A
positive atomic proposition is necessarily associated with one of the
three judgments $\mtrue$, $\meph$, or $\mpers$, but pseudo-positive
...
have concluded ${\sf robot} \fuse {!}({\sf 6bucks} \lefti \{ {\sf
  battery} \})$ instead of simply ${\sf robot}$. It is also possible
...
and reformulates the usual properties of cut and identity in terms of
...
term language, a consideration that is completely orthogonal to this
...
done by encoding the substructural derivations in LF using Crary's
explicit contexts technique \cite{crary10higher} or they have been in
HLF, an extension to LF that uses equational theories to capture
substructural properties \cite{reed09hybrid}.
...
logic, and while we conjecture that a suitably-defined notion of multifocusing
for \ollll~would be in bijective correspondence with \sls~terms modulo
concurrent equivalence, at least if we omit unification. 

The interactions between unification and concurrent equality are quite
...
{\bullet}S_2$ by giving ill-typed counterexamples. The violation
of either condition does not mean that $S_2; S_1$ will be ill-typed
however, so it might be possible to give a more precise condition
that admits a coarser notion of concurrent equality. 
