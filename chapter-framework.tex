...
al.~\cite{nanevski08contextual}. In particular, we write
$[\lf{t}/\lf{x}]$ for a simultaneous substitution that acts as the
identity on all variables except for $\lf{x}$; this notation is used
...
\cite{harper07mechanizing}. The judgments The judgments
$\lf{x}\#\lf{\sigma}$, $\lf{x}\#\Psi$, $\lf{\sf c}\#\Sigma$, ${\sf
  a}\#\Sigma$, and ${\sf r}\#\Sigma$ assert that the relevant variable
and constant does not already appear in the context $\Psi$ (as a
binding $\lf{x}{:}\tau$), the signature $\Sigma$ (as a declaration
($\lf{\sf c} : \tau$, ${\sf a} : \nu$, or ${\sf r} : A^-$), or the
substitution $\lf{\sigma}$ (as a binding $\lf{t/x}$ or
\mbox{$\lf{y/\!\!/x}$}).
