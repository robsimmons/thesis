...
Generative signatures a significant extension of traditional grammars,
both because of the presence of dependent types and because of the
presence of linear and persistent resources in \sls. However, we will
not endeavor to student generative signatures in their own right in
this chapter or this thesis. Rather, we will use generative signatures
for one very specific purpose: showing that, under some generative
signature $\Sigma_{\it Gen}$ that defines a world $\mathcal W$,
whenever $(\Psi; \Delta) \in \mathcal W$ and $(\Psi; \Delta)
\leadsto_\Sigma (\Psi'; \Delta')$ it is always the case that $(\Psi';
\Delta') \in \mathcal W$.  (The signature $\Sigma$ encodes the
transition system we are studying.)  In such a case, the world or
language of well-formed process states is called a {\it generative
  invariant} of $\Sigma$.

\subsection*{Type preservation}

Narrowing our focus even further, in this chapter our sole use of
generative invariants in this chapter is describing well-formedness
...
Safety The preservation proof in a standard ``safety = progress +
preservation'' formulation of type safety is, of course, a statement
about invariants.

Well-typedness invariants are important because they allow us to
...
Preservation in an SOS specification says that if $\obj{e
  \mapsto e'}$ and $\obj{e}$ has type $\obj{\it tp}$, then $\obj{e'}$
has type $\obj{\it tp}$ as well -- the type is invariant under
evaluation.
...
$\obj{e}$ to be free of undefined behavior. 
...
The action of an inversion lemma is to conclude, based on the
...
\begin{proof}
  ...
  more generic formulation of the proof which avoids this case
  ...
  \item[Case $S = \trstep{z}{{\sf gen/eval}\,\lf{e}\,x''}$]~
    ...
    We have that $\tackon{\Theta}{y_1{:}\istrue{\susp{{\sf gen}}}, ~
      y_2{:}\istrue{\susp{{\sf gen}}}, ~ y_3{:}\istrue{\susp{{\sf
            cont}\,\lf{f}}}} = \tackon{\Theta'}{z{:}\istrue{\susp{{\sf
            eval}\,\lf{e}}}}$.  
    ...
    two-hole contexts here could be done following the structure of
...
but they deserve mention for two reasons. First, they are of paramount
importance in the context of the logical framework CLF, a framework
that lacks \sls's notions of order. Second, destination-passing
inspired the progress and preservation proofs for distributed systems
that were consdered by Cervesato and Sans
\cite{cervesato13substructural}; this section establishes that their
reasoning can be formalized in the context of the \sls~framework.
