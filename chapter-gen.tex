...
One advantage of consumptive signatures is that it lets us work with
...
\[\left(\lf{l_1}{:}{\sf mutable\_loc},\ldots,\lf{l_n}{:}{\sf mutable\_loc};
  ...
  ~~ \ldots\right)\]
%
Naively, we might attempt to describe well-formed process 
mutable state as a regular world specification
with a rule like ${\sf gen/cell/bad} : \forall \lf{v}.\, {!}{\sf
  value}\,\lf{v} \lefti \{ \exists \lf{l}. {\sf cell}\,\lf{l}\,\lf{v}
\}$. The problem with such a specification is that it makes it
impossible to have a cell refer to itself, a situation that can easily
happen in practice. The canonical example, using back-patching to
implement recursion, is shown in Figure~\ref{fig:bigbackpatch}, which
...
\[{\sf some} ~ \lf{\it tp}{:}{\sf
  typ} ~ {\sf block} ~ \lf{l}{:}{\sf mutable\_loc}, x{:}\ispers{{\sf
...
\\ $T' :: (\cdot; x{:}\iseph{\susp{{\sf gen}\,\lf{{\it tp}_0}}})
       \leadsto^*_{\siggenstate}
       (\Psi'; \tackon{\Theta}
...
\medskip
\item If $T :: (\cdot; x{:}\istrue{\susp{{\sf gen}\,\lf{{\it tp}_0}\,\lf{d_0}}})
         \leadsto^*_{\siggenstate} 
         (\Psi; \tackon{\Theta}
...
\\ where $x_t{:}\ispers{\susp{{\sf of}\,\lf{l}\,\lf{\it tp}}} \in \Delta$,
...
maintained, we need one property besides inversion, which I will call
the {\it unique index} property. This is the property that, under the
generative signature described by $\siggenstate$, locations always map
{\it uniquely} to persistent positive propositions 
$x_t{:}{\sf ofcell}\,\lf{l}\,\lf{\it tp}$. 
