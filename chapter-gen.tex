...
transitions $(\Psi; \Delta) \leadsto^* (\Psi'; \Delta')$.\footnote{See
  Appendix~\ref{appendix-procsum} for a summary of the notation used for
  steps, traces, process states, and frames.} This chapter will focus on
another view of concurrent \sls~specifications as {\it grammars} for
describing well-formed process states. This view was presented
...
special case of the worlds described by generative signatures.  
After this, we essentially play the same game 

play out a similar proof of preservation
five t

\smallskip
\begin{itemize}
\item In Section~\ref{sec:gen-order} we extend the well-formedness
  invariant for sequential ordered abstract machines
...
\item In Section~\ref{sec:gen-state} we describe how generative
  ...
  requires us to consider {\it uniqueness} properties of
...
\item In Section~\ref{sec:gen-destinations} we consider invariants for
  ...
  from \cite{cervesato13substructural}, 
\item In Section~\ref{sec:gen-letcc} we consider the peculiar case of
  ...
  Section~\ref{sec:dest-continuations}.
\end{itemize}
\smallskip
...
generally (Section~\ref{sec:gen-order}). 
 In
...
Chapter~\ref{chapter-destinations}, are not a focus of this dissertation,
