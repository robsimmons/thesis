...
\\ where 
   $\cdot \vdash_{\siggenorder} N : \isconc{\susp{{\sf value}\,\lf{v}}}$.
\medskip
\item If 
   $T :: (x{:}\istrue{\susp{\sf gen}}) \leadsto^*_{\siggenorder}
...
\begin{proof}
  ...
  \isconc{\susp{{\sf okf}\,\lf{f}}}$ by traditional type inversion
...
\begin{proof} As in the proofs of Theorem~\ref{thm:pda-preservation}
  ...
  $T_2$. We give a few illustrative cases.

\begin{description}
\item 
  [$\trstep{y}{{\sf ev/unit}\,x}
   ::
   \tackon{\Theta}{x{:}\istrue{\susp{{\sf eval}\,\lf{\sf unit}}}}
   \leadsto
   \tackon{\Theta}{y{:}\istrue{\susp{{\sf retn}\,\lf{\sf unit}}}}$]~

Applying inversion (Part 1) to the given trace, we have 
...
\>\>$\tackon{\Theta}{x'{:}\istrue{\susp{\sf gen}}}$
\\
\>$\trstep{x}{{\sf gen/eval}\,\lf{\sf unit}\,x'}$
\\
\>\>$\tackon{\Theta}{x{:}\istrue{\susp{{\sf eval}\,\lf{\sf unit}}}}$
\end{tabbing}
...
the LF encoding of an expression
still has the LF type ${\sf exp}$.
