...
application) in Chatper~\ref{chatper-destinations}. To use sequential
...
When we move from orderd abstract machines to destination-passing
...
\begin{enumerate}
\item
If $T :: (\lf{d_0}{:}{\sf dest}; x_0{:}\istrue{\susp{{\sf gen}\,\lf{{\it tp}_0}}})
...
\begin{proof}
Induction and case analysis on last steps of the trace $T$. 
\end{proof}
...
cases analagous to the ones described in
...
\\ then 
   $T = \left(T';
       \trstep{y}{{\sf gen/retn}\,\lf{{\it tp}}\,\lf{d}\,\lf{e}
...
\\ then 
   $T = \left(T';
       \trstep{y}{{\sf gen/cont}\,\lf{{\it tp}}\,\lf{d}\,\lf{f}\,\lf{{\it tp}'}
...
\subsection{Preservation}
 ...
 First, it allows us to hew to the 
cardinal rule that inversion lemmas never produce 


isolated invariants can be used to describe the invariants 
...
Parallel
evaluation (Figure~\ref{fig:dest-pair}) and the alternate semantics of
parallelism and faiulre (Figure~\ref{fig:dest-fail-paror}) are
straightforward extensions to the development in this section.
Synchronization (Section~\ref{sec:dest-synch}) and futures
(Section~\ref{sec:dest-futures}) are just a bit more intersting from
the perspective of generative invariants and presesrvation, but we
will not develop them here.\footnote{A {\it progress} theorem for a
  language with synchronization, which would fall under the scope of
  Chapter~\ref{chapter-safety}, would require reasoning about
  deadlocks and is oslsutside the scope of this thesis.}
