...
is derived from the language of logic. Specifications in a logical
framework therefore have an independent meaning and existence given by
the logic from which the logical framework was derived. For example,
Coq is based on the Calculus of Inductive Constructions
\cite{coq10coq}, Agda is based on a variant of Martin-L\"of's type
theory called ${\sf UTT}_\Sigma$ \cite{norell08towards}, and Twelf is
based off of the dependent type theory $\lambda^\Pi$, also known as LF
...
communication, or lazy evaluation -- that are not particularly pleasant
to specify using existing logical frameworks. Dealing with a few
unpleasant features at a time might not be much trouble. However, the
combinations that appear in actual programming languages result in
formal specifications that are neither clear for humans nor convenient
for formal tools to reason about. A more precise statement is that the
addition of the aforementioned features is {\it non-modular}, because
handling a new feature requires reconsidering and revising all the
other rules.

Some headway on this problem has been made by frameworks like the K
semantic framework that are formal but not logically derived; the K
semantic framework is based on a notion of rewriting rules
\cite{rosu10overview}.  (Ellison and Ro\c{s}u's formalization of C was
done in the K semantic framework.)
...
interpretation as rewriting rules (as observed by Cervesato and
Scedrov \cite{cervesato09relating}). Deriving a framework from
substructural logics can allow us to combine the formality and
generality of logical frameworks with the modular specification that
is possible in rewriting frameworks. With this synthesis, we aim to
...
methodology by which we organize various styles of specification
through the logical correspondence. Part~3 discusses formal reasoning
about properties of \sls~specifications, with an emphasis
on establishing invariants.
...
game) for a single-state push-down automaton for checking that a
string has only matching angle braces by using two rewriting rules:
\begin{align}
