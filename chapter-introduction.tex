...
linguistic~\cite{lambek58mathematics}. In ordered logic, hypotheses
have an intrinsic notion their position relative to one another, and
therefore they cannot be rearranged. The rewriting rules we considered
above can be explained as propositions in ordered logic, where the
...
partially evaluated value: $\obj{f} = \obj{\Box\,e_2}$ represents an
expression $\obj{e_1\,e_2}$ waiting on the evaluation of $\obj{e_1}$
to a value, and $\obj{f} = \obj{(\lambda x.e)\,\Box}$ represents
...
Chapter~\ref{chapter-correspondence}.
...
in Chapter~\ref{chapter-correspondence}. In Chapter~\ref{chapter-absmachine}, 
we show how we can use the logical
...
semantics do not support combining the specification of a pure
...
we can specify pure features (like call-by-value
...
preserve more specific invariants. An important class of examples are
invariants about the types of expressions and process states, which
will be considered in Chapter~9. For almost any SSOS specification
more complicated than the one given above, type invariants are
necessary for proving the {\it progress theorem}, which implies that
the specification is safe -- that is, free from undefined behavior.
Chapter~10 will consider the use of generative invariants for proving
safety properties of specifications.

\section{Contributions}

We support our central thesis in the three
...
\noindent
The first major contribution of Part 1 of the thesis are the
development of {\it structural focalization}
\cite{simmons11structural}, the application of structural focalization
...
\noindent
The major contribution of Part 2 is the development of the {\it
...
semantics that we introduced in Section~\ref{sec:intro-ssos}, and the
destination-adding transformation given in
Chapter~\ref{chapter-destinations} connects these specifications with
the older {\it destination-passing style} SSOS specification. The
transformations in both chapters add new opportunities for modular
extension -- that is, our ability to add new features to the language
specification without revising existing rules.

\smallskip
\begin{quote} 
  {\bf Thesis (Part III):} {\it \sls~specifications of the operational
    semantics of programming language are a suitable basis for formal
    reasoning about the properties of those programming languages.}
\end{quote} 
\smallskip 

\noindent
We discuss two techniques for formal reasoning about the properties of
SSOS specifications in \sls. The use of generative signatures to
describe invariants is discussed in Chapter~\ref{chapter-gen}, and the
use of these invariants to prove safety properties of programming
languages is discussed in Chapter~10. Abstract interpretation is
a different kind of formal reasoning about operational semantics
specifications. In Chapter~\ref{chapter-approx} we discuss the logical
approximation methodology and show that it can be used to take SSOS
specifications and derive known control flow and alias analyses that
