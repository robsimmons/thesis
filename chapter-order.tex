...
$\foc{\Psi'}{\tackon{\lf\sigma\Theta}{[\lf{\sigma}(\lf{[t/a]}A^-)]}}
  {\lf\sigma{U}}$; the result 
follows by the induction hypothesis on $\lf\sigma$ and
$\Sp$.
...
%% In unfocused \ollll, using the compound notation
...
three parts (for
substituting into values, terms, and spines). % \cite{simmons11structural}.
...
\item[--] $\lsubsta{\tappr{V}{\Sp}}{M}{A^+} 
  ...
  By invoking the induction hypothesis to substitute $M$ out of $\Sp$, we have
  $\lsubsta{\Sp}{M}{A^+}$, which is a derivation of
...
\item[--] $\rsubsta{M}{x}{(\tfocusl{x}{\Sp})}{A^-} =$\\
  ...
  as $\Theta'$ has an extra variable declaration $x{:}\ispers{A^-}$. Instead,
  we know 
...
\item[--] $\rsubsta{M}{x}{\tappr{V}{\Sp}}{A^-} =$\\
  ...
  In the second case ($x$ is in $\Theta'$ only),
  $\Xi$ matches 
  ...
  In the third case ($x$ is in $\Theta'$ and $\Delta_A$), 
  $\Xi$ matches 
  $\frameoff{\Theta_\Delta}{\matchconj{[B_1^+ \righti B_2^-]}{\Delta_A'}}$,
...
\item[--] $\etapa{z}{N}{{\downarrow}A^-} 
  ...
  propositions from $\Delta$. This means  
...
\infer-
{\foct{\Psi}{\Delta}
  {\lsubsta{N_1}{\etapa{z}{\tfocusr{\tinl{z}}}{A^+}}{A^+}}
  {\islvl{A^+ \oplus B^+}}}
...
logical frameworks has been to define proof terms $E$ that have little, if
...
define $\rsubst{M}{x}{(\tfuser{V_1}{V_2})}$ as substituting $M$
into both $V_1$ and $V_2$, as we are
