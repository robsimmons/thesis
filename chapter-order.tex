...
\cite{polakow01ordered}. It extends linear logic with a notion of {\it
  ordered resources}.  In ordered logic, for example, the linear
...
nnecessary to construct an $A$, can construct a $B$, splits into two
...
given a noun phrase to its right, constructs a noun phrase (``ideas''
is a noun phrase, and so is ``clever ideas''). Therefore, the world
``clever'' can be seen as an ordered resource ${\sf NounPharse \righti
  NounPhrase}$. Similarly, the word ``quietly'' is an adverb that,
given a verb phrase to its left, constructs a verb phrase (``sleeps''
is a verb phrase, and so is ``sleeps quietly''). Therefore, the word
``quietly'' can be seen as an ordered resource ${\sf VerbPhrase \lefti
...
is an resource derivable from the persistent resources in $\Gamma$,
...
\]
Conversely, if we know $\islax{A}$ the only thing we can prove is
...
\end{align*}
The connectives and $\one$, $\zero$, $A \oplus B$, $\top$, $A \with B$
...
\lf{a}{:}\tau. A$, $\forall \lf{a}{:}\tau$, and $\mid \lf{t}
...
logical transformations presented in Chapter~7 and in the program
analysis methodology in Chapter~8. The left rule for equality $\lf{t}
...
unifier $\lf{t}$ and $\lf{s}$ (a unifier is any substitution
...
this higher-order formulation at face value: the premise is actually a
(meta-level) mapping -- a function -- that takes a substitution
$\lf{\sigma}$, the codomain $\Psi'$ of that substitution, and any
evidence necessary show that $\lf{\sigma}$ unifies $\lf{t}$ and
...
unifier, we can choose to define the logic with two rules $\doteq_{\it
  no}$ and $\doteq_{\it yes}$ is equivalent to defining the logic with
$\doteq_L$; the 
...
terms $\tau$ are dependently typed and therefore may include terms
...
these contexts primarily serve to obscure the logic's presentation
...
bunched implications \cite{pym02semantics} could also be given a
...
The second basic operation on contexts requires a new concept, {\it
  frames} $\Theta$. Intuitively, we can view frames as a collection
persistent, linear, and ordered contexts where the ordered context is
missing a particular piece.  Informally, we can write this missing
...
gives us the space to treat syntactically sequences of variable
declarations as equivalent. As an example, $(x{:}\iseph{A},
...
presented in the remainder of this section will rely on or preserve
the ordering of ephemeral variable declarations.
...
the focusing constraint discussed in Chapter 2: there is only ever at
...
it into our development, but the meaning of this class, as well as the
...
development of traces as a syntax for partial proofs in Chapter~4.
...
${\sf id}^+$ and ${\doteq}_R$, where we write out the matching
...
{\frameoff{\Theta}{\restrictto{\Delta}{\mlvl}} \altv \istrue{\isconc C}}
{\Delta \altv \isconc{A}
 &
 \tackon{\Theta}{x{:}\islvl{A}} \altv \isconc{C}}
\]
...
positive cut formulae and part 2 is for negative ct formula. The third
...
cut admissibility theorem in Chapter 2 (Theorem~\ref{thm:lincut}),
...
tedious, quadratically large confluence theorem discussed for linear
...
constructs. The computational content of parts 1 and 2 is {\it principal
 substitution}, written as $(\subst{V}{N})^{A^+}$ and $(\subst{N}{\Sp})^{A^-}$,
the computational content of part 3 is {\it leftist substitution},
written as $\lsubsta{E}{N}{A^+}$, and the computational content 
of part 4 is {\it rightist substitution}, written as 
$\rsubsta{M}{x}{E}{A^-}$.
...
arbitrary permeable propositions). The computational contents of 
the admissible rules $\eta_{A^+_\mlvl}$ and $\eta_{A^-_\mlvl}$ are two functions
$\etapa{z}{N}{A^+_\mlvl}$ and $\etana{N}{A^-_\mlvl}$.
\[
\infer-[\eta_{A^+_\mlvl}]
...
{\foct{\Psi}{\Delta}{N}{\islvl{\langle A^-_\mlvl \rangle}}
 ...
 \stableL{\Delta}}
\]
We define these functions in the proof of
Theorem~\ref{thm:ordidexpand} below. Identity expansion is not the
...
\begin{proof} By mutual induction over the structure of types. We
  ...
  cases that are very similar to others.\robnote{I think this cases of
    this proof that I wrote out in detail need to be redone entirely
    in the style of the ${\downarrow}A^-$ case, but I want to get
    feedback before I do that.  For now, I've left the rest of this
    proof alone (July 19).}
...
\item[--] $\etapa{z}{N}{{!}A^-}
  ...
  $\Theta'$ is $\Theta$ plus an additional variable declaration 
